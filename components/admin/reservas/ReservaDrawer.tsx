...
import {
  X, Ticket, User, CalendarDays, CheckCircle2,
  XCircle, CreditCard, Phone, Mail, LogIn, Clock,
} from 'lucide-react'
      ...
      <aside className="fixed right-0 top-0 z-50 h-full w-full max-w-[380px] bg-gray-50 shadow-2xl flex flex-col">
        <div className="bg-white border-b border-gray-100 px-5 py-4 flex items-center justify-between shrink-0">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <Badge variant="outline" className={cn('text-[10px]', ESTADO_BADGE[reserva.estado])}>
                {reserva.estado}
              </Badge>
              {reserva.ingresado && (
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          <div className="bg-brand-azul/5 border border-brand-azul/20 rounded-2xl p-4 text-center">
            <p className="text-xs text-brand-azul font-bold uppercase tracking-wide">Fecha del evento</p>
            <p className="text-xl font-black text-brand-azul mt-1">
              {format(parseISO(reserva.fechaEvento), "EEEE d 'de' MMMM yyyy", { locale: es })}
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-0.5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">{reserva.edadNino} anos de edad</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-4">
            ...
            <Row icon={User}        label="Acompanante"  value={reserva.nombreAcompanante} />
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-4">
            ...
            {reserva.referenciaPago && (
              <p className="text-xs font-mono text-gray-400 mt-1">{reserva.referenciaPago}</p>
            )}
          </div>
