...
export function SeguridadTab({ configs }: { configs: ConfiguracionSistema[] }) {
  ...
  const { register, handleSubmit, reset, formState: { errors, isDirty } } = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      EXPIRACION_SESION_ADMIN_MIN:  Number(map.EXPIRACION_SESION_ADMIN_MIN ?? 30),
    },
  })
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div className="flex items-center gap-2 mb-1">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Control de intentos fallidos y expiración de sesiones</p>
        </div>
      </div>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-muted-foreground">Número de contraseñas incorrectas antes de bloquear la cuenta.</p>
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-muted-foreground">Tiempo que permanecerá bloqueada la cuenta tras los intentos fallidos.</p>
          {errors.DURACION_BLOQUEO_LOGIN_MIN && <p className="text-xs text-destructive">{errors.DURACION_BLOQUEO_LOGIN_MIN.message}</p>}
        </div>

        <div className="space-y-1.5">
          <Label htmlFor="EXPIRACION_SESION_ADMIN_MIN">Expiración de sesión</Label>
          <div className="flex items-center gap-2">
            <Input id="EXPIRACION_SESION_ADMIN_MIN" type="number" min={5} className="max-w-[120px]" {...register('EXPIRACION_SESION_ADMIN_MIN')} />
            <span className="text-sm text-muted-foreground">minutos de inactividad</span>
          </div>
          <p className="text-xs text-muted-foreground">Tiempo máximo de inactividad antes de cerrar la sesión del administrador.</p>
      ...
      <div className="rounded-xl bg-amber-50 border border-amber-100 p-4 text-sm text-amber-800">
        <strong>Nota:</strong> Los cambios en seguridad afectan a todos los administradores del sistema.
