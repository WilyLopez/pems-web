...
const CATALOGOS: CatalogGroup[] = [
  {
    ...
    items: ['PENDIENTE', 'CONFIRMADA', 'CANCELADA', 'REPROGRAMADA', 'COMPLETADA'],
  },
  {
    titulo: 'Estado de evento privado',
    descripcion: 'Estados de los eventos contratados por empresas o familias',
    items: ['SOLICITADO', 'EN_REVISION', 'APROBADO', 'RECHAZADO', 'CANCELADO', 'REALIZADO'],
  },
  {
  ...
  },
  {
    ...
    items: ['DESCUENTO_PORCENTAJE', 'DESCUENTO_FIJO', 'ENTRADA_GRATIS', 'COMBO'],
  },
  {
    titulo: 'Categoría de producto',
    descripcion: 'Grupos de productos del inventario',
    items: ['ALIMENTO', 'BEBIDA', 'DECORACION', 'HIGIENE', 'ENTRETENIMIENTO', 'OTRO'],
  },
]
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Valores de referencia fijos — no editables desde la interfaz</p>
        </div>
      </div>

      <div className="rounded-xl bg-blue-50 border border-blue-100 p-3 text-sm text-blue-700">
        Estos catálogos son valores del dominio del negocio definidos a nivel de base de datos.
        Si necesitas modificarlos, contacta al equipo de desarrollo.
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        {CATALOGOS.map(grupo => (
          <div key={grupo.titulo} className="rounded-xl border border-gray-100 p-4 space-y-3">
            <div>
              <p className="text-sm font-semibold text-gray-900">{grupo.titulo}</p>
