...
const schema = z.object({
  ...
  INTERVALO_PREPARACION_INICIO: z.string().regex(/^\d{2}:\d{2}$/, 'Formato HH:MM'),
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Hora de apertura y cierre del local (formato 24 h)</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-1.5">
          ...
          <Input id="HORA_APERTURA" type="time" {...register('HORA_APERTURA')} />
          {errors.HORA_APERTURA && <p className="text-xs text-destructive">{errors.HORA_APERTURA.message}</p>}
        </div>

        <div className="space-y-1.5">
          <Label htmlFor="HORA_CIERRE">Hora de cierre</Label>
          <Input id="HORA_CIERRE" type="time" {...register('HORA_CIERRE')} />
          {errors.HORA_CIERRE && <p className="text-xs text-destructive">{errors.HORA_CIERRE.message}</p>}
        </div>

        <div className="space-y-1.5">
          <Label htmlFor="AFORO_MAXIMO">Aforo máximo (personas)</Label>
          <Input id="AFORO_MAXIMO" type="number" min={1} {...register('AFORO_MAXIMO')} />
          {errors.AFORO_MAXIMO && <p className="text-xs text-destructive">{errors.AFORO_MAXIMO.message}</p>}
        </div>
      </div>

      <div className="pt-2 border-t border-gray-100">
        <p className="text-sm font-medium text-gray-700 mb-3">Intervalo de preparación entre turnos</p>
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-1.5">
            <Label htmlFor="INTERVALO_PREPARACION_INICIO">Inicio del intervalo</Label>
            <Input id="INTERVALO_PREPARACION_INICIO" type="time" {...register('INTERVALO_PREPARACION_INICIO')} />
            {errors.INTERVALO_PREPARACION_INICIO && <p className="text-xs text-destructive">{errors.INTERVALO_PREPARACION_INICIO.message}</p>}
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="INTERVALO_PREPARACION_FIN">Fin del intervalo</Label>
            <Input id="INTERVALO_PREPARACION_FIN" type="time" {...register('INTERVALO_PREPARACION_FIN')} />
            {errors.INTERVALO_PREPARACION_FIN && <p className="text-xs text-destructive">{errors.INTERVALO_PREPARACION_FIN.message}</p>}
          </div>
        </div>
      </div>
