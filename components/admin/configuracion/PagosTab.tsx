...
export function PagosTab({ configs }: { configs: ConfiguracionSistema[] }) {
  ...
  const { control, handleSubmit, reset, formState: { isDirty } } = useForm<FormValues>({
    ...
    defaultValues: { PSE_PROVEEDOR: (map.PSE_PROVEEDOR as (typeof PROVEEDORES)[number]) ?? 'NUBEFACT' },
  })

  useEffect(() => {
    const m = toMap(configs)
    reset({ PSE_PROVEEDOR: (m.PSE_PROVEEDOR as (typeof PROVEEDORES)[number]) ?? 'NUBEFACT' })
  }, [configs, reset])
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <div className="flex items-center gap-2 mb-1">
        ...
        <div>
          <h3 className="font-semibold text-gray-900">Pagos y facturación electrónica</h3>
          <p className="text-xs text-muted-foreground">Proveedor de servicios electrónicos SUNAT</p>
        </div>
      </div>

      <div className="max-w-sm space-y-1.5">
        <Label htmlFor="PSE_PROVEEDOR">Proveedor de servicios electrónicos</Label>
        <Controller
          control={control}
          name="PSE_PROVEEDOR"
          render={({ field }) => (
            <Select value={field.value} onValueChange={field.onChange}>
              ...
              <SelectContent>
                ...
                <SelectItem value="SUNAT_DIRECTA">SUNAT Directa (OSE)</SelectItem>
        ...
        <p className="text-xs text-muted-foreground">
          Plataforma utilizada para la emisión de comprobantes electrónicos ante SUNAT.
        </p>
      </div>
