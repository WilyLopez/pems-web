...
export function InfoPersonalForm({ admin }: { admin: UsuarioAdmin }) {
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-6">
      ...
      <form onSubmit={handleSubmit((v) => actualizar.mutate(v))} className="space-y-5">
        <div className="grid gap-5 sm:grid-cols-2">
          <FieldWrapper>
            ...
            <div className="relative">
              ...
              <Input
                id="nombre"
                className={cn('pl-9 h-10 rounded-xl', errors.nombre && 'border-red-300 focus-visible:ring-red-400')}
                {...register('nombre')}
              />
            </div>
            {errors.nombre && (
