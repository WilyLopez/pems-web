// components/admin/calendario/CalendarioDayDrawer.tsx
...
import {
  X, Ticket, PartyPopper, TrendingUp, AlertTriangle,
  Sun, Sunset, Users, CreditCard, Info, ChevronRight,
} from 'lucide-react'
import { useResumenDia } from '@/hooks/useCalendario'
...
function TurnoCard({
  ...
  const Icon = label === 'Manana' ? Sun : Sunset
  return (
    <div className={cn(
      ...
      <div className="flex items-center justify-between">
        ...
        <Badge variant="secondary" className={cn(
          'text-[10px]',
          turno.disponible           ? 'bg-green-100 text-green-800'
          : turno.eventoPrivado      ? 'bg-brand-rosa/15 text-brand-rosa'
          :                           'bg-orange-100 text-orange-800',
        )}>
          {turno.disponible ? 'Disponible' : turno.eventoPrivado ? 'Evento privado' : 'Ocupado'}
        </Badge>
      </div>
            ...
            <div className="p-5 space-y-4">
              ...
              {data.alertas.length > 0 && (
                <div className="space-y-2">
                  ...
                  {data.alertas.map((a, i) => <AlertaItem key={i} alerta={a} />)}
                </div>
              )}
              ...
              <div className="flex gap-2">
                <Button size="sm" className="flex-1 rounded-xl bg-brand-azul hover:bg-brand-azul/90 text-white text-xs gap-1.5" asChild>
                  <Link href={`/admin/reservas?fecha=${fecha}`}>
                    <Ticket className="h-3.5 w-3.5" />
                    Ver reservas
                  </Link>
                </Button>
                <Button size="sm" variant="outline" className="flex-1 rounded-xl border-brand-rosa text-brand-rosa hover:bg-brand-rosa/5 text-xs gap-1.5" asChild>
