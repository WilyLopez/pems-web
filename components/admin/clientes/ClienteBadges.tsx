...
export function EstadoBadge({ activo }: { activo: boolean }) {
  return (
    <span className="flex items-center gap-1.5">
      ...
      />
      <span className={cn('text-xs font-medium', activo ? 'text-green-700' : 'text-gray-400')}>
...
export function TipoBadge({ tipo }: { tipo?: string | null }) {
  if (tipo === 'EMPRESA') {
    return (
      <Badge variant="outline" className="gap-1 text-xs border-brand-azul/30 text-brand-azul">
  ...
  return (
    <Badge variant="outline" className="gap-1 text-xs text-gray-500 border-gray-200">
...
export function VisitasBadge({ visitas }: { visitas: number }) {
  const nivel =
    visitas >= 20 ? { label: 'Frecuente', cls: 'bg-green-100 text-green-800 border-green-200' }
    : visitas >= 5  ? { label: 'Regular',  cls: 'bg-brand-azul/10 text-brand-azul border-brand-azul/20' }
    :                 { label: `${visitas}`, cls: 'bg-gray-100 text-gray-600 border-gray-200' }
