...
function JsonBlock({ label, value }: { label: string; value?: string }) {
  ...
  let formatted = value
  try { formatted = JSON.stringify(JSON.parse(value), null, 2) } catch { /* raw */ }
  return (
    ...
    <div className="space-y-4 text-sm">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
          log.resultado === 'EXITOSO' ? 'bg-green-100 text-green-800' :
          log.resultado === 'FALLIDO' ? 'bg-red-100 text-red-800' : 'bg-orange-100 text-orange-800'
        }`}>{log.resultado}</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
        ...
        <div>
          ...
          <p>{log.entidadAfectada}{log.idEntidad ? ` #${log.idEntidad}` : ''}</p>
        </div>
        {log.ipOrigen && (
          <div>
      ...
      {(log.valorAnterior || log.valorNuevo) && (
        <>
          <Separator />
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Cambios registrados</p>
  ...
  return (
    <Dialog open={logId !== null} onOpenChange={(v) => { if (!v) onClose() }}>
      <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-base">Detalle del registro de auditoría</DialogTitle>
        </DialogHeader>
        ...
        {isLoading ? (
          ...
          <p className="text-sm text-muted-foreground py-4">No se pudo cargar el registro.</p>
        )}
      </DialogContent>
    </Dialog>
  )
