...
export function useRegistrarPagoReserva() {
  ...
  return useMutation({
    mutationFn: (payload: RegistrarPagoPayload) => pagoService.registrarReserva(payload),
    onSuccess: () => {
...
export function useRegistrarAdelanto() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (payload: RegistrarPagoPayload) => pagoService.registrarAdelanto(payload),
    onSuccess: () => {
