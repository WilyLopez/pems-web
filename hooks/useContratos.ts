...
import { contratoService, GenerarContratoPayload, ActualizarContratoPayload, CambiarEstadoPayload } from '@/services/contrato.service'
...
export function useGenerarContrato() {
  ...
  return useMutation({
    mutationFn: ({ idEvento, payload }: { idEvento: number; payload: GenerarContratoPayload }) =>
...
export function useActualizarContrato() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, payload }: { id: number; payload: ActualizarContratoPayload }) =>
