...
export function calcularIndicadores(evento: EventoPrivado): IndicadorEvento[] {
  ...
  if (!evento.precioTotalContrato) {
    indicadores.push({ tipo: 'CONTRATO', mensaje: 'Sin precio definido', nivel: 'DANGER' })
  }
  if (evento.montoSaldo && evento.montoSaldo > 0) {
    indicadores.push({ tipo: 'PAGO', mensaje: `Saldo S/ ${evento.montoSaldo.toFixed(2)}`, nivel: 'WARNING' })
  }
  if (!evento.checklistCompleto) {
    indicadores.push({ tipo: 'CHECKLIST', mensaje: 'Checklist incompleto', nivel: 'WARNING' })
  }
