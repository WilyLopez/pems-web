          ...
          <div className="flex-1 space-y-2">
            ...
            {evento.precioTotalContrato && (
              <div className="flex flex-wrap gap-4 text-sm pt-1">
                <span>
                  Total: <span className="font-semibold">{formatCurrency(evento.precioTotalContrato)}</span>
                </span>
                {evento.montoAdelanto !== undefined && evento.montoAdelanto > 0 && (
...
export default function MisEventosPage() {
  ...
  const { data, isLoading, isError, refetch } = useEventos({ page: 0, size: 20 })
    ...
    <div className="space-y-4">
      ...
      {isLoading && (
        <div className="space-y-3">
          {Array.from({ length: 3 }).map((_, i) => <EventoCardSkeleton key={i} />)}
        </div>
      )}
      ...
      {!isLoading && !isError && (
        (data?.content.length ?? 0) === 0 ? (
          ...
          <div className="space-y-3">
            {data?.content.map((e) => <EventoCard key={e.id} evento={e} />)}
          </div>
        )
      )}
    </div>
