...
import { z } from 'zod'
import {
  Plus, ToggleLeft, Tag, Calendar, TrendingUp, Users, DollarSign,
  Clock, AlertTriangle, CheckCircle2, BarChart3, Sparkles, EyeOff, Filter,
...
import {
  usePromociones, useCrearPromocion, useDesactivarPromocion, useEstadisticasPromocion,
...
function getEstadoVisual(p: Promocion): 'ACTIVA' | 'PROXIMA' | 'POR_VENCER' | 'EXPIRADA' {
...
const ESTADO_CONFIG = {
  ACTIVA:     { label: 'Activa',     cls: 'bg-green-100 text-green-800 border-green-200' },
  PROXIMA:    { label: 'Programada', cls: 'bg-blue-100 text-blue-800 border-blue-200' },
  POR_VENCER: { label: 'Por vencer', cls: 'bg-amber-100 text-amber-800 border-amber-200' },
  EXPIRADA:   { label: 'Inactiva',   cls: 'bg-gray-100 text-gray-500 border-gray-200' },
}
...
const TIPOS_PROMO = Object.entries(TIPO_LABEL).map(([value, label]) => ({ value, label }))
...
function PromocionCard({ promo, onDesactivar }: { promo: Promocion; onDesactivar: (p: Promocion) => void }) {
  const estado = getEstadoVisual(promo)
  const dias   = diasRestantes(promo.fechaFin)
  const cfg    = ESTADO_CONFIG[estado]

  return (
    <div className={cn('rounded-2xl border bg-white overflow-hidden hover:shadow-md transition-shadow', estado === 'EXPIRADA' && 'opacity-60')}>
      <div className="h-2 w-full" style={{ backgroundColor: promo.colorDestacado ?? '#00AEEF' }} />
      ...
      <div className="p-4 space-y-3">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <p className="font-bold text-gray-900 text-sm leading-tight truncate">{promo.nombre}</p>
            <p className="text-xs text-muted-foreground mt-0.5">{TIPO_LABEL[promo.tipoPromocion] ?? promo.tipoPromocion}</p>
          </div>
          <Badge variant="secondary" className={cn('text-[11px] shrink-0 border', cfg.cls)}>{cfg.label}</Badge>
        </div>

        <div className="text-2xl font-black" style={{ color: promo.colorDestacado ?? '#00AEEF' }}>
          {formatDescuento(promo)} OFF
        </div>

        {promo.textoPublicitario && (
          <p className="text-xs text-gray-600 leading-relaxed line-clamp-2">{promo.textoPublicitario}</p>
        )}

        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
          <Calendar className="h-3.5 w-3.5" />
          {formatDate(promo.fechaInicio)}{promo.fechaFin && ` — ${formatDate(promo.fechaFin)}`}
        </div>
        ...
        <div className="flex flex-wrap gap-1">
          {promo.mostrarEnInicio            && <span className="text-[10px] bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded">Inicio</span>}
          {promo.mostrarEnCarrusel          && <span className="text-[10px] bg-purple-50 text-purple-700 px-1.5 py-0.5 rounded">Carrusel</span>}
          {promo.mostrarEnCheckout          && <span className="text-[10px] bg-green-50 text-green-700 px-1.5 py-0.5 rounded">Checkout</span>}
          {promo.mostrarEnPaginaPromociones && <span className="text-[10px] bg-rose-50 text-rose-700 px-1.5 py-0.5 rounded">Web pública</span>}
        </div>
...
function StatsRow({ promociones }: { promociones: Promocion[] }) {
  const activas     = promociones.filter(p => getEstadoVisual(p) === 'ACTIVA').length
  const porVencer   = promociones.filter(p => getEstadoVisual(p) === 'POR_VENCER').length
  const programadas = promociones.filter(p => getEstadoVisual(p) === 'PROXIMA').length
  const inactivas   = promociones.filter(p => !p.activo).length

  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      {[
        { label: 'Activas',     value: activas,     icon: CheckCircle2,  color: 'text-green-600 bg-green-50' },
        { label: 'Por vencer',  value: porVencer,   icon: AlertTriangle, color: 'text-amber-600 bg-amber-50' },
        { label: 'Programadas', value: programadas, icon: Clock,         color: 'text-blue-600 bg-blue-50' },
        { label: 'Inactivas',   value: inactivas,   icon: EyeOff,        color: 'text-gray-500 bg-gray-50' },
...
function TabEstadisticas({ promociones }: { promociones: Promocion[] }) {
  ...
  const totalUsos      = stats?.totalClientesAlcanzados ?? promociones.reduce((s, p) => s + (p.vecesUsado ?? 0), 0)
  const totalDescuento = stats?.totalDescuentoOtorgado  ?? promociones.reduce((s, p) => s + (p.montoAhorrado ?? 0), 0)
  const totalClientes  = stats?.totalClientesAlcanzados ?? promociones.reduce((s, p) => s + (p.clientesAtraidos ?? 0), 0)

  const top = [...promociones].sort((a, b) => (b.vecesUsado ?? 0) - (a.vecesUsado ?? 0)).slice(0, 5)

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'Total de usos',       value: totalUsos,                      icon: TrendingUp, color: '#00AEEF' },
          { label: 'Clientes alcanzados', value: totalClientes,                  icon: Users,      color: '#F64B8A' },
          { label: 'Descuento otorgado',  value: formatCurrency(totalDescuento), icon: DollarSign, color: '#F59E0B' },
        ].map(({ label, value, icon: Icon, color }) => (
          <Card key={label} className="border-0 shadow-sm">
      ...
      <Card className="border-0 shadow-sm">
        ...
        <CardContent>
          {top.length === 0 ? (
            <p className="text-sm text-muted-foreground py-4 text-center">Sin datos de uso aún.</p>
          ) : (
            <div className="space-y-3">
              {top.map((p, i) => {
                const maxUsos = top[0].vecesUsado ?? 1
                const pct = maxUsos > 0 ? ((p.vecesUsado ?? 0) / maxUsos) * 100 : 0
                return (
                  <div key={p.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <p className="text-xs text-muted-foreground shrink-0 ml-2">{p.vecesUsado ?? 0} usos</p>
                      </div>
                      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div
                          className="h-full rounded-full transition-all"
                          style={{ width: `${pct}%`, backgroundColor: p.colorDestacado ?? '#00AEEF' }}
                        />
                      </div>
                    </div>
                  </div>
...
function CrearPromocionDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  const crear = useCrearPromocion()
  const [dialogTab, setDialogTab] = useState('basico')

  const { register, handleSubmit, setValue, watch, reset, formState: { errors } } =
    useForm<PromoFormValues>({ resolver: zodResolver(promoSchema), defaultValues: DEFAULT_VALUES })

  const zonaValues = ZONAS.map(z => watch(z.key as keyof PromoFormValues) as boolean)
  ...
  const onSubmit = (values: PromoFormValues) => {
    const payload: CrearPromocionPayload = {
      ...
      limitePorCliente:           values.limitePorCliente ? parseInt(values.limitePorCliente) : undefined,
      minimoAsistentes:           values.minimoAsistentes ? parseInt(values.minimoAsistentes) : undefined,
      montoMinimo:                values.montoMinimo ? parseFloat(values.montoMinimo) : undefined,
    }
    crear.mutate(payload, {
      onSuccess: () => { onOpenChange(false); reset(); setDialogTab('basico') },
    })
  }

  const handleClose = () => { onOpenChange(false); reset(); setDialogTab('basico') }
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs value={dialogTab} onValueChange={setDialogTab}>
          ...
          <TabsContent value="basico" className="space-y-4 pt-2">
            <div className="space-y-2">
              ...
              <Select onValueChange={(v) => setValue('tipoPromocion', v as TipoPromocion)}>
                ...
                <SelectContent>
                  {TIPOS_PROMO.map(t => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}
                </SelectContent>
              </Select>
              {errors.tipoPromocion && <p className="text-xs text-destructive">{errors.tipoPromocion.message}</p>}
            </div>

            <div className="space-y-2">
              <Label>Nombre</Label>
              <Input placeholder="Ej: Martes Familiar 2x1" {...register('nombre')} />
              {errors.nombre && <p className="text-xs text-destructive">{errors.nombre.message}</p>}
            </div>

            <div className="space-y-2">
              <Label>Descripción</Label>
              <Textarea rows={2} placeholder="Descripción breve..." {...register('descripcion')} />
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                {errors.valorDescuento && <p className="text-xs text-destructive">{errors.valorDescuento.message}</p>}
              </div>
              <div className="space-y-2">
                <Label>Aplica en</Label>
                <Select onValueChange={(v) => setValue('soloTipoDia', v as TipoDia)}>
                  <SelectTrigger><SelectValue placeholder="Todos los días" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value={TipoDia.SEMANA}>Solo días de semana</SelectItem>
                    <SelectItem value={TipoDia.FIN_SEMANA_FERIADO}>Fin de semana y feriados</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
            ...
            <label className="flex items-center gap-3 cursor-pointer select-none">
              <input type="checkbox" className="h-4 w-4 rounded accent-brand-azul" {...register('esAutomatica')} />
              <span className="text-sm font-medium">Aplicar automáticamente al reservar</span>
            </label>
          </TabsContent>
          ...
          <TabsContent value="visual" className="space-y-4 pt-2">
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-2">
                <Label className="flex items-center gap-1.5"><ImageIcon className="h-3.5 w-3.5" /> URL imagen tarjeta</Label>
                <Input placeholder="https://..." {...register('imagenUrl')} />
              </div>
              <div className="space-y-2">
                <Label className="flex items-center gap-1.5"><ImageIcon className="h-3.5 w-3.5" /> URL banner principal</Label>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <input type="color" {...register('colorDestacado')} className="h-9 w-12 rounded border p-1 cursor-pointer" />
                  <Input placeholder="#00AEEF" {...register('colorDestacado')} className="flex-1" />
                </div>
              </div>
              <div className="space-y-2">
                <Label>Prioridad (1–10)</Label>
                <Input type="number" min={1} max={10} {...register('prioridad')} />
              </div>
              <div className="space-y-2">
                <Label>URL botón de acción</Label>
                <Input placeholder="/zona-de-juegos" {...register('urlBoton')} />
              </div>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Textarea rows={2} placeholder="¡Oferta especial solo por hoy!" {...register('textoPublicitario')} />
              </div>
              <div className="space-y-2">
                <Label>Texto del botón CTA</Label>
                <Input placeholder="Reservar ahora" {...register('textoBoton')} />
              </div>
            </div>

            <div className="space-y-2">
              <Label className="flex items-center gap-1.5"><Globe className="h-3.5 w-3.5" /> Mostrar en la web</Label>
              <div className="grid grid-cols-2 gap-2">
                {ZONAS.map((zona, idx) => (
                  <label key={zona.key} className="flex items-center gap-2 cursor-pointer select-none p-2.5 rounded-lg border hover:bg-gray-50 transition-colors">
                    <input
                      type="checkbox"
                      className="h-4 w-4 rounded accent-brand-azul"
                      checked={!!zonaValues[idx]}
                      onChange={e => setValue(zona.key as keyof PromoFormValues, e.target.checked)}
                    />
          ...
          <TabsContent value="condiciones" className="space-y-4 pt-2">
            <p className="text-xs text-muted-foreground">Deja en blanco los límites que no apliquen.</p>
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-2">
                <Label>Límite total de usos</Label>
                <Input type="number" min={1} placeholder="Sin límite" {...register('limiteUsos')} />
              </div>
              <div className="space-y-2">
                <Label>Límite por cliente</Label>
                <Input type="number" min={1} placeholder="Sin límite" {...register('limitePorCliente')} />
              </div>
              <div className="space-y-2">
                <Label>Mínimo de asistentes</Label>
                <Input type="number" min={1} placeholder="Sin mínimo" {...register('minimoAsistentes')} />
              </div>
              <div className="space-y-2">
                <Label>Monto mínimo de compra (S/)</Label>
                <Input type="number" step="0.01" min={0} placeholder="Sin mínimo" {...register('montoMinimo')} />
              </div>
            </div>
          </TabsContent>
        </Tabs>
        ...
        <TabsContent value="promociones" className="space-y-5 pt-2">
            ...
            {(['todas', 'activas', 'inactivas'] as Filtro[]).map(f => (
              <button
                ...
                className={cn(
                  ...
                  filtro === f ? 'bg-brand-azul text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200',
                )}
              >
                {f}
              </button>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p className="font-medium">No hay promociones {filtro !== 'todas' ? filtro : 'configuradas'}.</p>
              <Button size="sm" className="mt-4" onClick={() => setOpenDialog(true)}>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {filtradas
                ...
                .map(p => <PromocionCard key={p.id} promo={p} onDesactivar={setDesactivarTarget} />)}
            </div>
          )}
        </TabsContent>
