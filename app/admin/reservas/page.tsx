...
import {
  Search, RefreshCw, Ticket, Users, XCircle, LogIn,
  TrendingUp, BarChart2, X, Filter, CheckCircle2,
  ArrowUpDown, Eye,
} from 'lucide-react'
import { useAuth } from '@/hooks/useAuth'
import { useDebounce } from '@/hooks/useDebounce'
import {
  useReservas, useMetricasReservas,
  useCancelarReserva, useConfirmarIngreso,
...
export default function ReservasPage() {
  ...
  const { data, isLoading, isError, refetch } = useReservas({
    page, size: 20, idSede: idSede ?? undefined,
  ...
  const handleEstado = (v: string) => { setEstado(v as EstadoReserva); setPage(0) }
  const closeDialog  = () => setDialog(null)

  const columns: ColumnDef<Reserva>[] = [
    {
      accessorKey: 'numeroTicket',
      header: () => <span className="text-xs font-bold text-gray-400 uppercase">Ticket</span>,
      cell: ({ row }) => (
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-sm text-gray-700">{formatDate(row.original.fechaEvento)}</span>
      ),
    },
    {
      accessorKey: 'estado',
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-1.5">
          <Badge
            variant="outline"
            className={cn('text-[10px] font-bold', ESTADO_BADGE[row.original.estado])}
          >
            {row.original.estado}
          </Badge>
    ...
    },
    {
      ...
      header: () => <span className="text-xs font-bold text-gray-400 uppercase">Nino</span>,
      cell: ({ row }) => (
        <div>
          <p className="text-sm font-semibold text-gray-900">{row.original.nombreNino}</p>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-sm text-gray-700">{row.original.nombreCliente ?? '—'}</span>
      ),
    },
    {
      accessorKey: 'totalPagado',
      header: () => <span className="text-xs font-bold text-gray-400 uppercase">Pago</span>,
      cell: ({ row }) => (
        <div>
          ...
          {row.original.medioPago && (
            <p className="text-[10px] text-gray-400">{row.original.medioPago}</p>
          )}
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const puedeIngreso = !r.ingresado && ['PENDIENTE', 'CONFIRMADA'].includes(r.estado)
        return (
    ...
    <div className="space-y-5">
      ...
      {metricas && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          <MetricaCard icon={Ticket}    label="Total del dia" value={metricas.totalReservas}
            color="bg-brand-azul/10 text-brand-azul" />
          <MetricaCard icon={BarChart2} label="Pendientes"    value={metricas.pendientes}
            color="bg-yellow-100 text-yellow-700" />
          <MetricaCard icon={XCircle}   label="Canceladas"    value={metricas.canceladas}
            color="bg-red-100 text-red-600" />
          <MetricaCard icon={LogIn}     label="Ingresados"    value={metricas.ingresados}
            color="bg-green-100 text-green-700" />
          <MetricaCard icon={TrendingUp}label="Ingresos"
      ...
      <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          {([
            ...
            <button
              key={String(value)}
              onClick={() => { setIngresado(value); setPage(0) }}
              className={cn(
      ...
      <ConfirmDialog
        ...
        onConfirm={() => dialog?.reserva && ingreso.mutate(dialog.reserva.id, { onSettled: closeDialog })}
      />
      ...
      <ConfirmDialog
        ...
        onConfirm={() => dialog?.reserva && cancelar.mutate(
