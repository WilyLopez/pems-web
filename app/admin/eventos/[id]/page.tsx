...
import {
  ChevronLeft, PartyPopper, User, CalendarDays, Users, Clock,
  CreditCard, FileText, ClipboardList, CheckCircle2, Circle,
  AlertTriangle, TrendingUp, Loader2,
} from 'lucide-react'
import { useEvento, useChecklist, useCompletarTarea, useDescompletarTarea } from '@/hooks/useEventos'
...
function InfoRow({ icon: Icon, label, value }: {
  icon: React.ElementType; label: string; value?: string | null | React.ReactNode
}) {
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline" className={cn('text-xs font-bold', ESTADO_BADGE[evento.estado])}>
                  {evento.estado}
                </Badge>
              </div>
                      ...
                      <>
                        <div className="space-y-3">
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <span className={cn(
                              'text-lg font-black',
                              (evento.montoSaldo ?? 0) > 0 ? 'text-amber-700' : 'text-green-700',
                            )}>
                        ...
                        <div className="space-y-1">
                          <div className="flex justify-between text-xs text-gray-400">
                            ...
                            <span>
                              {evento.precioTotalContrato > 0
                                ? Math.round(((evento.montoAdelanto ?? 0) / evento.precioTotalContrato) * 100)
                                : 0}%
                            </span>
                          </div>
                          <div className="h-2 w-full rounded-full bg-gray-100">
                            <div
                              ...
                              style={{
                                width: `${evento.precioTotalContrato > 0
                                  ? Math.min(((evento.montoAdelanto ?? 0) / evento.precioTotalContrato) * 100, 100)
                                  : 0}%`,
                              }}
                            />
                          </div>
                          ...
                          {checklist.map((item) => (
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <p className={cn('text-sm font-semibold', item.completada ? 'line-through text-gray-400' : 'text-gray-900')}>
                                  {item.tarea}
                                </p>
