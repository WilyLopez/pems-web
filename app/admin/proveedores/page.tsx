...
import { proveedorService, Proveedor, GestionarProveedorPayload } from '@/services/proveedor.service'
...
export default function ProveedoresPage() {
  ...
  const [dialog, setDialog] = useState<{ open: boolean; proveedor: Proveedor | null }>({
  ...
  const guardar = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(dialog.proveedor ? 'Proveedor actualizado.' : 'Proveedor creado.')
  ...
  const openCreate = () => { reset(); setDialog({ open: true, proveedor: null }) }
  ...
  const columns: ColumnDef<Proveedor>[] = [
    {
      ...
      cell: ({ row }) => (
        <div>
          ...
          {row.original.ruc && <p className="text-xs text-muted-foreground">RUC: {row.original.ruc}</p>}
        </div>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-sm">
          ...
          {row.original.contactoTelefono && (
            <p className="text-xs text-muted-foreground">{row.original.contactoTelefono}</p>
          )}
        </div>
      ),
    },
    {
      ...
      header: 'Estado',
      cell: ({ row }) => (
        <Badge variant="secondary" className={row.original.activo ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-500'}>
  ...
  return (
    <div className="space-y-4">
      ...
      <Dialog open={dialog.open} onOpenChange={(o) => { setDialog({ open: o, proveedor: null }); reset() }}>
        <DialogContent className="sm:max-w-lg">
          ...
          <form onSubmit={handleSubmit((v) => guardar.mutate(v))} className="space-y-3 py-2">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label>RUC <span className="text-muted-foreground">(opcional)</span></Label>
                <Input placeholder="20000000001" maxLength={11} {...register('ruc')} />
              </div>
              <div className="space-y-2">
                <Label>Tipo de servicio</Label>
                <Input placeholder="Ej: Catering, Animación" {...register('tipoServicio')} />
                {errors.tipoServicio && <p className="text-sm text-destructive">{errors.tipoServicio.message}</p>}
              </div>
              <div className="space-y-2">
                <Label>Nombre de contacto</Label>
                <Input placeholder="Juan Rodríguez" {...register('contactoNombre')} />
              </div>
              <div className="space-y-2">
                <Label>Teléfono de contacto</Label>
                <Input placeholder="987654321" {...register('contactoTelefono')} />
              </div>
              <div className="space-y-2 sm:col-span-2">
                <Label>Correo de contacto</Label>
                <Input type="email" placeholder="contacto@empresa.com" {...register('contactoCorreo')} />
              </div>
              <div className="space-y-2 sm:col-span-2">
                <Label>Notas internas <span className="text-muted-foreground">(opcional)</span></Label>
                <Textarea rows={2} {...register('notas')} />
              </div>
            </div>
          </form>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setDialog({ open: false, proveedor: null }); reset() }}>
              Cancelar
            </Button>
            <Button onClick={handleSubmit((v) => guardar.mutate(v))} disabled={guardar.isPending}>
