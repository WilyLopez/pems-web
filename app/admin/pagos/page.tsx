...
const pagoSchema = z.object({
  ...
  monto: z.string().min(1).refine((v) => parseFloat(v) > 0, 'Debe ser mayor a 0'),
...
function PagoForm({
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm<PagoFormValues>({
    resolver: zodResolver(pagoSchema),
  })

  const medioPago = watch('medioPago')
  const requiereReferencia = [MedioPago.YAPE, MedioPago.TRANSFERENCIA].includes(medioPago)
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <Input
            ...
            {...register('contextoId')}
          />
          {errors.contextoId && <p className="text-sm text-destructive">{errors.contextoId.message}</p>}
        </div>
        ...
        <div className="space-y-2">
          ...
          {errors.medioPago && <p className="text-sm text-destructive">{errors.medioPago.message}</p>}
        </div>

        {requiereReferencia && (
          <div className="space-y-2">
            <Label>N° de operación / referencia</Label>
            <Input placeholder="Ej: 123456789" {...register('referenciaPago')} />
          </div>
        )}
      </div>
    ...
    <div className="space-y-4">
      ...
      <Tabs defaultValue="reserva">
        ...
        <TabsContent value="reserva" className="mt-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Pago de reserva pública</CardTitle>
              <CardDescription>Registra el pago único de una reserva de acceso al local</CardDescription>
            </CardHeader>
            <CardContent>
              <PagoForm tipo="reserva" onSubmit={handleReserva} isPending={pagoReserva.isPending} />
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="adelanto" className="mt-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Adelanto de evento privado</CardTitle>
              <CardDescription>Registra el adelanto o pago parcial de un evento privado confirmado</CardDescription>
            </CardHeader>
            <CardContent>
              <PagoForm tipo="adelanto" onSubmit={handleAdelanto} isPending={adelanto.isPending} />
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
