...
import { z } from 'zod'
import { Monitor, Smartphone, Image as ImageIcon, Type, Link2, Settings, Shield, Zap } from 'lucide-react'
import { Banner, CrearBannerPayload, ActualizarBannerPayload } from '@/services/cms.service'
...
const schema = z.object({
  ...
  imagenUrl: z.string().min(1, 'La URL de imagen es obligatoria').url('Debe ser una URL válida'),
  enlaceDestino: z.string().url('Debe ser una URL válida').optional().or(z.literal('')),
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto p-0">
        <DialogHeader className="px-6 pt-6 pb-0">
          <DialogTitle>{isEditing ? 'Editar banner' : 'Nuevo banner'}</DialogTitle>
        </DialogHeader>

        <div className="grid lg:grid-cols-2 gap-0">
          {/* ── Formulario (Constructor de bloques) ── */}
          <form
            id="banner-form"
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <div>
                ...
                />
                {errors.imagenUrl && (
                  <p className="text-xs text-destructive mt-1">{errors.imagenUrl.message}</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div>
                ...
                {errors.enlaceDestino && (
                  <p className="text-xs text-destructive mt-1">{errors.enlaceDestino.message}</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  />
                  {errors.fechaInicio && (
                    <p className="text-xs text-destructive mt-1">{errors.fechaInicio.message}</p>
                  )}
                </div>
                <div>
          ...
          <div className="px-6 py-4 bg-gray-50 flex flex-col gap-4">
            ...
            <div className="flex justify-center">
              <div
                className={`bg-gradient-to-br from-[#001a2c] via-[#003a5c] to-[#001a2c] rounded-2xl overflow-hidden transition-all duration-300 ${
                  device === 'desktop' ? 'w-full aspect-[4/3]' : 'w-[200px] aspect-[9/16]'
              ...
              >
                {values.imagenUrl ? (
                  <div className="relative w-full h-full rounded-xl overflow-hidden border border-white/20 shadow-lg bg-gradient-to-br from-brand-azul/30 to-brand-rosa/30">
                    ...
                    {values.titulo && (
                      <div className="absolute top-2 right-2 bg-brand-amarillo text-gray-900 rounded-xl px-2 py-1 text-xs font-black shadow rotate-3 flex items-center gap-1">
                        ...
                        <span className="max-w-[80px] truncate">{values.titulo}</span>
                      </div>
                    )}
                  ...
                  <div className="flex flex-col items-center gap-2 text-white/40">
                    ...
                    <p className="text-xs text-center">Ingresa una URL de imagen para previsualizar</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {(values.titulo || values.fechaInicio) && (
              <div className="bg-white rounded-lg border p-3 space-y-1.5">
                {values.titulo && (
                  <p className="text-sm font-semibold text-gray-900 truncate">{values.titulo}</p>
                )}
        ...
        <div className="flex justify-end gap-2 px-6 py-4 border-t bg-white">
          ...
          >
            {isLoading ? 'Guardando...' : isEditing ? 'Guardar cambios' : 'Crear banner'}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
