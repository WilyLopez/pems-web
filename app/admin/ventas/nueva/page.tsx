...
import { ShoppingCart, Plus, Minus, Trash2, Receipt, Loader2 } from 'lucide-react'
...
export default function NuevaVentaPage() {
  ...
  const { items, addItem, removeItem, updateCantidad, clear, subtotal } = useCartStore()
  ...
  const procesar = useMutation({
    mutationFn: () =>
      ventaService.procesar(idSede ?? 1, {
        ...
        lineas: items.map((i) => ({ idProducto: i.idProducto, cantidad: i.cantidad })),
        descuento: parseFloat(descuento || '0'),
      }),
    onSuccess: (v) => {
      toast.success(`Venta #${v.id} procesada. Total: ${formatCurrency(v.total)}`)
      clear()
      setIdReserva('')
      setDescuento('0')
    },
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Nueva venta" description="Registra una venta de productos en el local" />
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-4">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Vincular a reserva <span className="text-muted-foreground font-normal text-xs">(opcional)</span></CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <CardContent className="space-y-4">
            {items.length === 0 ? (
              ...
              <div className="space-y-2">
                {items.map((item) => (
                  <div key={item.idProducto} className="flex items-center gap-2 text-sm">
                    <span className="flex-1 truncate">{item.nombre}</span>
                    <div className="flex items-center gap-1">
                      <Button
                        variant="ghost" size="icon" className="h-6 w-6"
                        onClick={() => updateCantidad(item.idProducto, item.cantidad - 1)}
                      >
