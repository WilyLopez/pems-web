...
import { Reserva, MetricasReserva, CrearReservaPayload, ReprogramarReservaPayload } from '@/types/reserva.types'
...
export const reservaService = {
  listar: async (params: {
    ...
    idSede?: number; fecha?: string
  }): Promise<PagedResponse<Reserva>> => {
    const { data } = await api.get<ApiResponse<PagedResponse<Reserva>>>('/reservas', { params })
    return data.data
  },

  buscarAdmin: async (params: BuscarReservasParams): Promise<PagedResponse<Reserva>> => {
    const { data } = await api.get<ApiResponse<PagedResponse<Reserva>>>(
      '/reservas/admin', { params })
    return data.data
  },

  metricas: async (idSede?: number, fecha?: string): Promise<MetricasReserva> => {
    const { data } = await api.get<ApiResponse<MetricasReserva>>(
      '/reservas/admin/metricas', { params: { idSede, fecha } })
    return data.data
  },

  crear: async (idCliente: number, idSede: number,
    payload: CrearReservaPayload): Promise<Reserva> => {
    const { data } = await api.post<ApiResponse<Reserva>>(
      `/reservas/clientes/${idCliente}/sedes/${idSede}`, payload)
    return data.data
  },

  reprogramar: async (idReserva: number,
    payload: ReprogramarReservaPayload): Promise<Reserva> => {
    const { data } = await api.post<ApiResponse<Reserva>>(
      `/reservas/${idReserva}/reprogramar`, payload)
    return data.data
  },
  ...
  confirmarIngreso: async (idReserva: number): Promise<Reserva> => {
    const { data } = await api.post<ApiResponse<Reserva>>(`/reservas/${idReserva}/ingreso`)
    return data.data
  },
}