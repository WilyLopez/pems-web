...
export const cmsService = {
  ...
  // ── Banners ────────────────────────────────────────────────────────────
  listarBanners: async (page = 0, size = 20): Promise<PagedResponse<Banner>> => {
    const { data } = await api.get<ApiResponse<PagedResponse<Banner>>>('/banners', {
      params: { page, size },
    })
    return data.data
  },
  ...
  actualizarBanner: async (id: number, payload: ActualizarBannerPayload): Promise<Banner> => {
    const { data } = await api.put<ApiResponse<Banner>>(`/banners/${id}`, payload)
    return data.data
  },
  ...
  duplicarBanner: async (id: number): Promise<Banner> => {
    const { data } = await api.post<ApiResponse<Banner>>(`/banners/${id}/duplicar`)
    return data.data
  },
