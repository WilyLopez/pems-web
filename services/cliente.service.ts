...
export const clienteService = {
  ...
  listar: async (
    ...
    const { data } =
      await api.get<ApiResponse<PagedResponse<Cliente>>>(
        '/clientes',
        { params },
      )
  ...
  obtener: async (
    ...
    const { data } =
      await api.get<ApiResponse<Cliente>>(
        `/clientes/${id}`,
      )
  ...
  actualizar: async (
    ...
    const { data } =
      await api.put<ApiResponse<Cliente>>(
        `/clientes/${id}`,
        payload,
      )
  ...
  hacerVip: async (
    ...
    const { data } =
      await api.post<ApiResponse<Cliente>>(
  ...
  quitarVip: async (
    ...
    const { data } =
      await api.delete<ApiResponse<Cliente>>(
        `/clientes/${id}/vip`,
      )
