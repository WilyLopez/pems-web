...
export default function IngresosPage() {
  ...
  const { data: rangeLista = [], isLoading: loadingRango } =
    useIngresosPorRango(
      filtroActivo ? (idSede ?? undefined) : undefined,
      desde || undefined,
      hasta || undefined
  ...
  const filteredRange = filtroActivo
    ? rangeLista.filter((e) => {
        ...
        if (q) {
          ...
          const nombre =
            tipos.find((t) => t.codigo === e.tipoIngresoCodigo)?.nombre ?? ''
          if (
  ...
  const ingresos = filtroActivo
    ...
    : (paginado?.content ?? [])
  const totalResults = filtroActivo
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <PageHeader
          title="Ingresos"
          description="Historial de ingresos registrados en la sede"
        />
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 text-xs text-gray-400">
                        <ArrowUpCircle
                          className={cn(
                            'h-3.5 w-3.5',
                            e.esAutomatico ? 'text-brand-azul' : 'text-gray-400'
                          )}
                        />
        ...
        {!isLoading && totalResults > 0 && (
          <div className="flex items-center justify-between px-4 py-3 border-t gap-4 flex-wrap">
            <p className="text-xs text-gray-400">
              {page * PAGE_SIZE + 1}–
              {Math.min((page + 1) * PAGE_SIZE, totalResults)} de {totalResults}{' '}
              registros
            </p>
            {totalPagesDisplay > 1 && (
      ...
      <Sheet
        open={!!detailIngreso}
        onOpenChange={(o) => !o && setDetailIngreso(null)}
      >
        <SheetContent
          side="right"
            ...
            <>
              <div className="shrink-0 bg-emerald-50 border-b border-emerald-100 px-5 py-5">
                ...
                <p className="text-sm text-emerald-600 mt-1.5">
                  {formatFecha(detailIngreso.fecha)}
                </p>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
                ...
                {detailIngreso.medioPago && (
                  <InfoRow label="Medio de pago">
                    {MEDIOS_PAGO.find(
                      (m) => m.value === detailIngreso.medioPago
                    )?.label ?? detailIngreso.medioPago}
                  </InfoRow>
                )}
                    ...
                    {detailIngreso.idReservaPublica && (
                      <InfoRow label="Reserva pública">
                        <span className="font-semibold">
                          N° {detailIngreso.idReservaPublica}
                        </span>
                      </InfoRow>
                    )}
                    {detailIngreso.idEventoPrivado && (
                      <InfoRow label="Evento privado">
                        <span className="font-semibold">
                          N° {detailIngreso.idEventoPrivado}
                        </span>
                      </InfoRow>
                    )}
                  </div>
                ...
                <div className="border-t border-gray-100 pt-4">
                  <p className="text-xs text-gray-400">
                    Registrado el{' '}
                    {new Date(detailIngreso.fechaCreacion).toLocaleString(
                      'es-PE'
                    )}
                  </p>
                </div>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-2">
            <div className="space-y-1">
              ...
              {errors.tipoIngresoCodigo && (
                <p className="text-xs text-red-500">
                  {errors.tipoIngresoCodigo.message}
                </p>
              )}
            </div>
