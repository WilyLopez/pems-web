...
import {
  crearReservaSchema,
  CrearReservaFormValues,
} from '@/lib/validations/reserva.schema'
...
export default function FormularioReservaPage() {
  ...
  const crear = useMutation({
    mutationFn: (values: CrearReservaFormValues) =>
      reservaService.crear(parseInt(session?.user?.id ?? '0'), SEDE_ID, values),
        ...
        >
          <div className="space-y-4">
            ...
            <div className="flex items-start gap-3">
              ...
              >
                He leído y acepto los términos de uso. Confirmo que soy mayor de
                edad y acompañaré al niño durante toda la visita.
              </Label>
            </div>
            {errors.firmoConsentimiento && (
              <p className="text-xs text-destructive">
                {errors.firmoConsentimiento.message}
              </p>
            )}
          </div>
        </Section>
