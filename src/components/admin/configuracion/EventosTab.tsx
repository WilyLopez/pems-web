...
export function EventosTab({ configs }: { configs: ConfiguracionSistema[] }) {
  ...
  const {
    ...
    defaultValues: {
      ANTICIPACION_MIN_EVENTO_PRIVADO_D: Number(
        map.ANTICIPACION_MIN_EVENTO_PRIVADO_D ?? 15
      ),
    },
  })
  ...
  useEffect(() => {
    ...
    reset({
      ANTICIPACION_MIN_EVENTO_PRIVADO_D: Number(
        m.ANTICIPACION_MIN_EVENTO_PRIVADO_D ?? 15
      ),
    })
  }, [configs, reset])
  ...
  function onSubmit(values: FormValues) {
    actualizar.mutate({
      ANTICIPACION_MIN_EVENTO_PRIVADO_D: String(
        values.ANTICIPACION_MIN_EVENTO_PRIVADO_D
      ),
    })
  }
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Parámetros de anticipación para contratación de eventos
          </p>
        </div>
      </div>

      <div className="max-w-sm space-y-1.5">
        <Label htmlFor="ANTICIPACION_MIN_EVENTO_PRIVADO_D">
          Anticipación mínima para solicitar un evento
        </Label>
        ...
        <p className="text-xs text-muted-foreground">
          Los clientes deben solicitar el evento con al menos este número de
          días de antelación.
        </p>
        {errors.ANTICIPACION_MIN_EVENTO_PRIVADO_D && (
          <p className="text-xs text-destructive">
            {errors.ANTICIPACION_MIN_EVENTO_PRIVADO_D.message}
          </p>
        )}
      </div>
