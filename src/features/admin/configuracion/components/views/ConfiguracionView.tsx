...
import { useConfiguracion, useConfiguracionCalendario } from '../../hooks/useConfiguracionData'
import { useConfiguracionNav, SECCION_LABELS } from '../../hooks/useConfiguracionNav'
...
export function ConfiguracionView() {
  ...
  const breadcrumbItems = [
    { label: 'Configuración', href: seccion ? '/admin/configuracion' : undefined },
    ...
    <div className="space-y-6">
      ...
      {!idSede && !isLoading && (
        <Alert variant="destructive">
          <AlertDescription>
            Tu usuario no tiene una sede asignada. Contacta al administrador del sistema para continuar.
          </AlertDescription>
        </Alert>
      )}
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <OperacionSection       config={configCalendario} idSede={idSede} navProps={navPropsFor('operacion')} />
            <ReservasEventosSection config={configCalendario} idSede={idSede} navProps={navPropsFor('reservas-eventos')} />
            <SedeSection            idSede={idSede}           navProps={navPropsFor('sede')} />
            <SeguridadSection       configs={configs}         navProps={navPropsFor('seguridad')} />
            <SistemaSection         configs={configs}         navProps={navPropsFor('sistema')} />
          </div>
        </>
      )}
    </div>
