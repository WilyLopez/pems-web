...
export function SystemHealthPanel({ idSede, configs }: SystemHealthPanelProps) {
  ...
  if (configPublica?.mantenimientoActivo) {
    alerts.push({
      level: 'warning',
      message:
        'El sitio web está en modo mantenimiento. Los visitantes no pueden acceder.',
  ...
  if (sede && !sede.ruc) {
    alerts.push({
      level: 'info',
      message:
        'La sede no tiene RUC registrado. Es obligatorio para emitir comprobantes electrónicos.',
  ...
  if (alerts.length === 0) {
    return (
      <div className="flex items-center gap-2.5 rounded-xl bg-green-50 border border-green-100 dark:bg-green-950/30 dark:border-green-900 px-4 py-2.5 text-sm text-green-700 dark:text-green-400">
          ...
          >
            ...
            {alert.action && (
              <Button
                ...
                onClick={() =>
                  abrirSeccion(alert.action!.seccion, alert.action!.modal)
                }
              >
                {alert.action.label}
              </Button>
