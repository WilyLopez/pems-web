      ...
      <div className="border-t border-border pt-4">
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          <MapaPickerDynamic
            ...
            longitud={lngNum}
            onChange={(lat, lng) => {
              setValue('latitud',  String(parseFloat(lat.toFixed(7))), { shouldDirty: true, shouldValidate: true })
              setValue('longitud', String(parseFloat(lng.toFixed(7))), { shouldDirty: true, shouldValidate: true })
            }}
          />
        </div>
        {latNum != null && lngNum != null && (
          <div className="mt-2 flex items-center gap-2.5 rounded-lg border border-green-100 bg-green-50 px-3 py-2">
            ...
            <div>
              <p className="text-xs font-semibold text-green-800">Ubicación registrada</p>
...
export function SedeSection({ idSede, navProps }: { idSede: number; navProps?: SeccionNavProps }) {
  ...
  const summary = sede ? [
    ...
    { label: 'Ciudad', value: sede.ciudad      },
    { label: 'RUC',    value: sede.ruc ?? '—'  },
    { label: 'Mapa',   value: sede.latitud != null ? 'Configurado' : 'Sin coordenadas' },
  ] : []
