...
function SeguridadForm({ configs }: { configs: ConfiguracionSistema[] }) {
  ...
  const {
    ...
    defaultValues: {
      ...
      EXPIRACION_SESION_CLIENTE_MIN: Number(
        m.EXPIRACION_SESION_CLIENTE_MIN ?? 60
      ),
    },
  })
  ...
  useEffect(() => {
    ...
    reset({
      ...
      EXPIRACION_SESION_ADMIN_MIN: Number(
        map.EXPIRACION_SESION_ADMIN_MIN ?? 120
      ),
      EXPIRACION_SESION_CLIENTE_MIN: Number(
        map.EXPIRACION_SESION_CLIENTE_MIN ?? 60
      ),
    })
  }, [configs, reset])
  ...
  function onSubmit(values: FormValues) {
    actualizar.mutate({
      ...
      EXPIRACION_SESION_CLIENTE_MIN: String(
        values.EXPIRACION_SESION_CLIENTE_MIN
      ),
    })
  }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-muted-foreground">
            Contraseñas incorrectas antes de bloquear la cuenta.
          </p>
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-muted-foreground">
            Tiempo que permanece bloqueada la cuenta.
          </p>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-muted-foreground">
            Cierra la sesión del administrador tras la inactividad indicada.
          </p>
        ...
        <div className="space-y-1.5">
          <Label htmlFor="EXPIRACION_SESION_CLIENTE_MIN">
            Sesión de cliente
          </Label>
          <div className="flex items-center gap-2">
            <Input
              id="EXPIRACION_SESION_CLIENTE_MIN"
              type="number"
              min={5}
              className="max-w-[120px]"
              {...register('EXPIRACION_SESION_CLIENTE_MIN')}
            />
            <span className="text-sm text-muted-foreground">
              min de inactividad
            </span>
          ...
          <p className="text-xs text-muted-foreground">
            Cierra la sesión del cliente en el portal web.
          </p>
          {errors.EXPIRACION_SESION_CLIENTE_MIN && (
            <p className="text-xs text-destructive">
              {errors.EXPIRACION_SESION_CLIENTE_MIN.message}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="rounded-xl bg-amber-50 border border-amber-100 p-4 text-sm text-amber-800">
        <strong>Nota:</strong> Los cambios afectan a todos los administradores.
        Los bloqueos activos no se modifican retroactivamente.
      </div>
...
export function SeguridadSection({
  ...
  const summary = [
    {
      label: 'Intentos antes bloqueo',
      value: m.INTENTOS_LOGIN_ANTES_BLOQUEO ?? '—',
    },
    {
      label: 'Sesión admin',
      value: m.EXPIRACION_SESION_ADMIN_MIN
        ? `${m.EXPIRACION_SESION_ADMIN_MIN} min`
        : '—',
    },
    {
      label: 'Sesión cliente',
      value: m.EXPIRACION_SESION_CLIENTE_MIN
        ? `${m.EXPIRACION_SESION_CLIENTE_MIN} min`
        : '—',
    },
  ]
  ...
  const viewItems = [
    ...
    {
      label: 'Duración bloqueo',
      value: m.DURACION_BLOQUEO_LOGIN_MIN
        ? `${m.DURACION_BLOQUEO_LOGIN_MIN} min`
        : '—',
    },
    {
      label: 'Sesión admin',
