...
function SistemaForm({ configs }: { configs: ConfiguracionSistema[] }) {
  ...
  const { control, register, handleSubmit, watch, reset, setValue, formState: { isDirty } } = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      SUNAT_PROVEEDOR:      (m.SUNAT_PROVEEDOR as typeof PROVEEDORES[number]) ?? 'NUBEFACT',
    },
  })
  ...
  useEffect(() => {
    ...
    if (config) {
      reset({
        ...
        SUNAT_PROVEEDOR:      (map.SUNAT_PROVEEDOR as typeof PROVEEDORES[number]) ?? 'NUBEFACT',
      })
    }
...
export function SistemaSection({ configs, navProps }: { configs: ConfiguracionSistema[]; navProps?: SeccionNavProps }) {
  const { data: config } = useConfiguracionAdmin()
  const m = toConfigMap(configs)
  const activo = config?.mantenimientoActivo ?? false

  const summary = [
    { label: 'Estado del sitio',  value: activo ? 'En mantenimiento' : 'Activo' },
