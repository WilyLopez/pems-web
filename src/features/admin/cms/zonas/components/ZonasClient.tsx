...
import { Plus, Pencil, Trash2, MapPin, Image as ImageIcon, Star, Search } from 'lucide-react'
            ...
            <tbody>
              {[1, 2, 3, 4].map((i) => (
                <tr key={i} className="border-b last:border-0">
                  ...
                  <td className="px-4 py-3 hidden sm:table-cell"><Skeleton className="h-3.5 w-16" /></td>
                  <td className="px-4 py-3 hidden md:table-cell"><Skeleton className="h-3.5 w-6 mx-auto" /></td>
                  <td className="px-4 py-3 hidden md:table-cell"><Skeleton className="h-3.5 w-6 mx-auto" /></td>
                  <td className="px-4 py-3 hidden lg:table-cell"><Skeleton className="h-5 w-5 rounded-full mx-auto" /></td>
        ...
        filtered.length === 0 ? (
          ...
          <div className="overflow-hidden rounded-xl border border-border">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((z) => {
                  const isDeleting = eliminar.isPending && eliminar.variables === z.id
                  return (
                        ...
                        <div className="flex items-center gap-3">
                          {z.imagenes.length > 0 ? (
                            <div className="relative w-10 h-10 rounded-lg overflow-hidden shrink-0">
                              <Image src={fixMediaUrl(z.imagenes[0])} alt={z.nombre}
