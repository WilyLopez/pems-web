...
export const novedadesSchema = z
  .object({
    ...
    prioridad: z.coerce
      .number()
      .min(0, 'Prioridad mínima 0')
      .max(100, 'Prioridad máxima 100')
      .default(0),
  ...
  .refine(
    ...
    {
      message:
        'Si habilitas la acción CTA, debes ingresar tanto el texto como el enlace de destino.',
      path: ['urlCta'],
    }
  )
  .refine(
...
export function NovedadFormDialog({
  ...
  const mostrarAlertaHome =
    visibleHome && novedadesHome >= 3 && !novedad?.visibleHome

  async function onSubmit(data: NovedadFormValues) {
    if (data.visibleHome && novedadesHome >= 3 && !isEditing) {
      toast.error(
        'Ya hay 3 novedades en el inicio. Desactiva una antes de agregar otra.'
      )
      return
    }
    setUploading(true)
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="flex flex-col lg:grid lg:grid-cols-[1fr_280px] gap-6">
            <div
              className={
                mobileTab === 'preview' ? 'hidden lg:block' : undefined
              }
            >
              <div className="space-y-4">
                ...
                <div className="flex flex-col gap-3 py-2 border-y border-gray-100 my-4">
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      {...register('hasCta')}
                      className="w-4 h-4 rounded text-brand-azul"
                    />
                    <span className="text-sm font-semibold text-gray-700">
                      Habilitar botón / acción de llamada a la acción (CTA)
                    </span>
                  </label>
                </div>
                ...
                {mostrarAlertaHome && (
                  <div className="rounded-lg bg-amber-50 border border-amber-200 p-3 text-xs text-amber-700 font-medium">
                    Solo se muestran 3 novedades en el inicio. Esta reemplazará
                    a la de menor prioridad.
                  </div>
                )}
            ...
            <div
              className={mobileTab === 'form' ? 'hidden lg:block' : undefined}
            >
