...
export const DrawerTurnoCard = React.memo(
  ({
    ...
    const permitirAsignar =
      !esPasado && disponiblePrivado && !tieneReservas && !turno.eventoPrivado
    ...
    return (
      <div className={cn('rounded-xl border p-3.5 space-y-2', borderCls)}>
        <div className="flex items-center justify-between">
          ...
          <Badge
            variant="secondary"
            className={cn('text-[10px] font-bold', badgeCls)}
          >
            {badgeLabel}
          </Badge>
        </div>
        ...
        {turno.eventoPrivado && (
          <div className="text-[11px] text-gray-600 space-y-1 pl-1">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              <span
                className={cn(
                  ...
                  estadoCls(turno.eventoPrivado.estado)
                )}
              >
            ...
            <p className="truncate text-gray-500">
              {turno.eventoPrivado.nombreCliente}
            </p>
        ...
        {!turno.eventoPrivado && tieneReservas && (
          <p className="text-[11px] text-gray-500 flex items-center gap-1.5 pl-1">
            ...
            {turno.totalReservas}{' '}
            {turno.totalReservas === 1 ? 'reserva' : 'reservas'}
          </p>
        )}
        ...
        {permitirAsignar && (
          <Button
            size="sm"
            variant="outline"
            className="h-7 text-[11px] px-2.5 rounded-lg gap-1 w-full"
            asChild
          >
            <Link
              href={`/admin/eventos/nuevo?fecha=${fecha}&turno=${turnoKey}`}
            >
