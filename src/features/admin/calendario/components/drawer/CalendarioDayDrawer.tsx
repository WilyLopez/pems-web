...
function resolveMode(
  ...
  if (tipoOcupacion === 'PRIVADO_PARCIAL' || tipoOcupacion === 'PRIVADO_LLENO') return 'EVENTO'
...
export const CalendarioDayDrawer = React.memo(({
  ...
  const tieneProgramacion = !esPasado && (disp?.tieneProgramacionSemanal ?? false)
  ...
  const badgeKey =
    mode === 'LIBRE'
      ? tieneProgramacion ? 'LIBRE_PROG' : 'LIBRE_NOPROG'
      : mode
  const badge =
  ...
  const turnoSection = (
    <div className="space-y-2">
      ...
      <DrawerTurnoCard
        turno={data?.turnoT1 ?? { disponible: false, totalReservas: 0, eventoPrivado: null }}
      ...
      />
      <DrawerTurnoCard
        turno={data?.turnoT2 ?? { disponible: false, totalReservas: 0, eventoPrivado: null }}
        label="Turno 2 — Tarde"
        ...
        <div className="bg-white border-b border-gray-100 px-4 py-3 shrink-0">
          <div className="flex items-center justify-between gap-2 mb-2">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className={cn('text-[11px] font-bold px-2.5 py-1 rounded-full whitespace-nowrap', badge.cls)}>
                {badge.label}
              </span>
          ...
          <p className="text-xs text-gray-400 mt-0.5">{format(date, 'yyyy')}</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin">
          ...
          {data && (
            <div className="p-5 space-y-4">
              ...
              {mode === 'PASADO' && (
                <>
                  <DrawerStats reservas={data.totalReservas} eventos={data.totalEventos} ingresos={data.ingresoEstimado} />
                  ...
                  {tieneActividad && <GastosSection idSede={idSede} fecha={fecha} />}
                </>
              )}
              ...
              {mode === 'EVENTO' && (
                <>
                  ...
                  {turnoSection}
                  <DrawerEventsList eventos={data.eventos} />
                  {data.pagosPendientes > 0 && <PagosBanner count={data.pagosPendientes} />}
