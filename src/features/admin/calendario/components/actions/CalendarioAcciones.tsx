...
const bloqueoSchema = z
  .object({
    fechaInicio: z
      ...
      .refine((v) => !v || v >= hoy, { message: 'No se pueden bloquear fechas pasadas' }),
  ...
  .refine(
    (v) => !v.fechaInicio || !v.fechaFin || v.fechaFin >= v.fechaInicio,
    { message: 'La fecha fin debe ser igual o posterior a la fecha inicio', path: ['fechaFin'] }
  )
...
const feriadoSchema = z.object({
  ...
  fecha: z
    ...
    .refine((v) => !v || v >= hoy, { message: 'No se pueden registrar feriados en fechas pasadas' }),
...
export const CalendarioAcciones = React.memo(({ idSede }: CalendarioAccionesProps) => {
  ...
  const { data: dispBloqueo } = useDisponibilidadRango(idSede, inicioBloqueo, finBloqueo || inicioBloqueo)
  const fechaFeriado = feriadoForm.watch('fecha')
  const { data: dispFeriado } = useDisponibilidadRango(idSede, fechaFeriado, fechaFeriado)
  ...
  const submitBloqueo = (values: BloqueoForm, confirmado = false) => {
    ...
    if (tieneProgramacionBloqueo && !values.esEmergencia) {
      toast.error('Este rango incluye fechas con programacion semanal activa. Usa el modo de emergencia.')
      return
    }
    bloquear.mutate(
      {
  ...
  return (
    <>
      ...
      <Dialog open={modal === 'bloqueo'} onOpenChange={(v) => !v && openModal(null)}>
          ...
          <DialogHeader className="px-6 py-5 bg-white border-b border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">Cierra el acceso publico en el rango seleccionado</p>
              </div>
            </div>
          </DialogHeader>

          <form onSubmit={handleBloqueo} className="px-6 py-5 space-y-4 bg-gray-50/40">
            ...
            {tieneAtencionBloqueo && (
              <div className="bg-white border border-red-200 rounded-xl p-3 flex items-start gap-2">
                ...
                <p className="text-xs text-red-800 font-medium">No se puede bloquear hoy con atencion activa en el local.</p>
              </div>
            )}
            ...
            {tieneProgramacionBloqueo && (
              <div className="flex items-center gap-3 bg-white border border-red-100 rounded-xl p-3">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => bloqueoForm.setValue('esEmergencia', !!checked)}
                />
                <div>
                  <label htmlFor="esEmergencia" className="text-xs font-bold text-red-900 cursor-pointer">
                    Caso de emergencia
                  </label>
                  <p className="text-[11px] text-red-600 mt-0.5">Permite bloquear fechas con programacion semanal activa.</p>
                </div>
              </div>
            )}
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-bold text-gray-600">Tipo de bloqueo</Label>
      ...
      <Dialog open={modal === 'feriado'} onOpenChange={(v) => !v && openModal(null)}>
          ...
          <DialogHeader className="px-6 py-5 bg-white border-b border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">Marca una fecha como feriado nacional o regional</p>
              </div>
            </div>
          </DialogHeader>

          <form onSubmit={handleFeriado} className="px-6 py-5 space-y-4 bg-gray-50/40">

            <div className="space-y-1.5">
              <Label className="text-xs font-bold text-gray-600">Tipo de feriado</Label>
            ...
            {tieneAtencionFeriado && (
              <div className="bg-white border border-red-200 rounded-xl p-3 flex items-start gap-2">
                <AlertTriangle className="h-4 w-4 text-red-500 shrink-0 mt-0.5" />
                <p className="text-xs text-red-800 font-medium">No se puede registrar feriado para hoy con atencion activa.</p>
              </div>
            )}
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-bold text-gray-600">Descripcion</Label>
              <Input
              ...
              {feriadoForm.formState.errors.descripcion && (
                <p className="text-xs text-destructive">{feriadoForm.formState.errors.descripcion.message}</p>
              )}
            </div>
