...
import {
  ...
  addDays, startOfDay, parseISO,
} from 'date-fns'
import { es } from 'date-fns/locale'
import {
  CalendarPlus, Loader2, ChevronLeft, ChevronRight,
  Info, AlertTriangle, Zap, X, CalendarCheck,
} from 'lucide-react'
...
export const ProgramacionSemanalModal = React.memo(({
  ...
  const { data: disponibilidad, isLoading: loadingDisp } = useDisponibilidadRango(idSede, inicio, fin)
  const { data: programaciones,  isLoading: loadingList } = useProgramaciones(idSede)
  ...
  const esSemanaActual  = inicioSemana.getTime() === inicioSemanaActual.getTime()
  const yaProgramada    = disponibilidad?.some(d => d.tieneProgramacionSemanal) ?? false
            ...
            {loadingList ? (
              ...
              <div className="space-y-2">
                {programaciones.map(p => {
                  ...
                  const isActual = yaInicio && semanaFinDate.getTime() >= hoy.getTime()
                  return (
                    <div
                      key={p.id}
                      className={cn(
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-gray-900 truncate">
                            {format(parseISO(p.semanaInicio), "d MMM", { locale: es })}
                            {' — '}
                            {format(parseISO(p.semanaFin), "d MMM yyyy", { locale: es })}
                          </p>
          ...
          <div className="px-6 pt-4 pb-6 space-y-4">
            ...
            {loadingDisp ? (
              ...
              <div className="bg-white border border-gray-100 rounded-xl p-6 flex flex-col items-center text-center gap-2">
                ...
                <p className="text-xs font-bold text-gray-500">Semana finalizada</p>
                <p className="text-[11px] text-gray-400">No se pueden hacer cambios en periodos pasados.</p>
              </div>
            ) : (
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          hayEvento ? 'border-pink-200 ring-1 ring-pink-100' : 'border-gray-100 shadow-sm',
                        )}
                      >
                        <p className="text-[9px] font-bold uppercase tracking-wider text-gray-400 capitalize">
                          {format(d, 'EEE', { locale: es })}
                        </p>
                        <p className="text-base font-black text-gray-900 my-1.5">{format(d, 'd')}</p>
                        <span className={cn(
                          'text-[8px] font-black uppercase leading-none px-1.5 py-0.5 rounded-md',
                          hayEvento ? 'bg-pink-50 text-pink-600' : 'bg-green-50 text-green-700',
                        )}>
                ...
                {esSemanaActual && !yaProgramada && (
                  <div className="flex items-start gap-2.5 bg-amber-50 border border-amber-100 rounded-xl px-3.5 py-3">
                    ...
                    <p className="text-[11px] text-amber-700 leading-relaxed">
                      Estás programando la <strong>semana en curso</strong>. Las reservas públicas se
                      habilitarán inmediatamente para los días restantes.
                    </p>
                  </div>
                )}
                ...
                {tieneEventos && (
                  <div className="bg-white border border-blue-100 rounded-xl px-4 py-3 flex items-start gap-2.5">
                    ...
                    <p className="text-[11px] text-gray-500 leading-relaxed">
                      ...
                      {diasConEvento.length === 1 ? 'celebración pactada' : 'celebraciones pactadas'}.
                      La programación habilita la venta pública sin afectar los eventos privados.
                    </p>
                  </div>
                )}
                ...
                {yaProgramada ? (
                  <div className="bg-white border border-blue-100 rounded-xl px-4 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-xs font-bold text-gray-900">Esta semana ya está programada</p>
                    </div>
