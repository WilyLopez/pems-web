...
import { z } from 'zod'
import {
  Settings, Loader2, Users, CalendarDays, Lock,
  Clock, Cake, HelpCircle, Save,
} from 'lucide-react'
...
const configSchema = z.object({
  aforoMaximo:           z.number().min(1, 'Mínimo 1 persona').max(500, 'Máximo 500 personas'),
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <div className={cn('h-6 w-6 rounded-lg flex items-center justify-center shrink-0', color)}>
...
export const ConfigurarCalendarioModal = React.memo(({
  ...
  const onSubmit = (values: ConfigForm) => {
    ...
    actualizar.mutate({ ...restConfig, ...values }, { onSuccess: () => onClose() })
  }
      ...
      <DialogContent className="sm:max-w-lg p-0 overflow-hidden rounded-2xl border border-gray-100 shadow-xl max-h-[90vh] flex flex-col">
        ...
        {isLoading ? (
          ...
          >
            ...
            {/* Reservas públicas */}
            <Section icon={CalendarDays} title="Reservas públicas" color="bg-emerald-500">
              <div className="bg-gray-50 rounded-xl p-4 space-y-4">
                <p className="text-[11px] text-gray-500 leading-relaxed">
                  Ventana de tiempo dentro de la cual los clientes pueden hacer reservas para el público general.
                </p>
                  ...
                  <div>
                    ...
                    <div className="relative mt-2">
                      <Input
                        ...
                        {...form.register('diasMinReservaPublica', { valueAsNumber: true })}
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-gray-400 pointer-events-none">días</span>
                    </div>
                    <FieldError message={err.diasMinReservaPublica?.message} />
                  </div>
                  <div>
                    <FieldLabel
                    ...
                    <div className="relative mt-2">
                      <Input
                        type="number"
                        className="bg-white pr-12"
                        {...form.register('diasMaxReservaPublica', { valueAsNumber: true })}
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-gray-400 pointer-events-none">días</span>
                    </div>
                    <FieldError message={err.diasMaxReservaPublica?.message} />
                  </div>
                </div>
              </div>
            </Section>
            ...
            {/* Eventos privados */}
            <Section icon={Lock} title="Eventos privados" color="bg-violet-500">
              <div className="bg-gray-50 rounded-xl p-4 space-y-4">
                <p className="text-[11px] text-gray-500 leading-relaxed">
                  Ventana de tiempo para agendar eventos privados (cumpleaños, corporativos, etc.).
                </p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <FieldLabel
                    ...
                    <div className="relative mt-2">
                      <Input
                        type="number"
                        className="bg-white pr-12"
                        {...form.register('diasMinEventoPrivado', { valueAsNumber: true })}
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-gray-400 pointer-events-none">días</span>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="relative mt-2">
                      <Input
                        type="number"
                        className="bg-white pr-12"
                        {...form.register('diasMaxEventoPrivado', { valueAsNumber: true })}
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-gray-400 pointer-events-none">días</span>
                    </div>
            ...
            {/* Horario */}
            <Section icon={Clock} title="Horario de operación" color="bg-orange-500">
              <div className="bg-gray-50 rounded-xl p-4 space-y-4">
                <p className="text-[11px] text-gray-500 leading-relaxed">
                  El sistema bloqueará reservas fuera de este horario. Pasada la hora de cierre, el día de hoy ya no aceptará reservas nuevas.
                </p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <FieldLabel
            ...
            <Section icon={Cake} title="Cumpleaños" color="bg-pink-500">
              <div className="bg-gray-50 rounded-xl p-4 space-y-4">
                <p className="text-[11px] text-gray-500 leading-relaxed">
                  Rango de edades que el local acepta para celebraciones de cumpleaños.
                </p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <FieldLabel
                    ...
                    <div className="relative mt-2">
                      <Input
                        ...
                        max={99}
                        className="bg-white pr-12"
                        {...form.register('edadMinCumple', { valueAsNumber: true })}
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-gray-400 pointer-events-none">años</span>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="relative mt-2">
                      <Input
                        ...
                        max={99}
                        className="bg-white pr-12"
                        {...form.register('edadMaxCumple', { valueAsNumber: true })}
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-gray-400 pointer-events-none">años</span>
                    </div>
