    ...
    <div className="w-52 rounded-xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-xl p-3 space-y-2.5">
      <div className="flex items-start justify-between gap-2">
        <p className="text-xs font-black text-gray-900 dark:text-gray-100 leading-tight">
          {format(day, "EEEE d 'de' MMMM", { locale: es }).replace(/^\w/, (c) =>
            c.toUpperCase()
          )}
        </p>
        {bloqueado && (
        ...
        <div className="space-y-1.5">
          ...
          {tipo === 'PRIVADO_PARCIAL' && (
            <p className="text-[10px] text-gray-400 font-medium">
              {!disp.turnoT1Ocupado
                ? 'Turno mañana disponible'
                : 'Turno tarde disponible'}
            </p>
          )}
...
export const CalendarioCelda = React.memo(
  ({ day, disp, loading, selected, onSelect }: CalendarioCeldaProps) => {
    if (loading) {
      return (
    ...
    const bgClass = cn(
      ...
      !selected &&
        !pasado &&
        feriado &&
        'bg-purple-50 border-purple-200 dark:bg-purple-950/30 dark:border-purple-900/60',
      !selected &&
        !pasado &&
        esPrivadoLleno &&
      ...
      !selected &&
        ...
        pct > 30 &&
        pct <= 70 &&
        'bg-yellow-50/60 border-yellow-200 dark:bg-yellow-950/20 dark:border-yellow-900/40',
      !selected &&
        !pasado &&
        !bloqueado &&
        !feriado &&
