...
export const CalendarioSemana = React.memo(({
  ...
  const days = eachDayOfInterval({ start: semanaInicio, end: addDays(semanaInicio, 6) })
    ...
    <div className="grid grid-cols-7 border-l border-t border-gray-100 rounded-xl overflow-hidden animate-fade-in">
      {days.map((day) => {
        ...
        return (
          ...
          >
            <div className={cn('text-center px-2 py-2.5 border-b border-gray-100', hoy && 'bg-brand-azul/5')}>
            ...
            <div className="flex-1 flex flex-col">
              ...
              {bloqueado && (
                <div className="flex items-center gap-1 px-2 pt-2 text-red-500">
                  ...
                  <span className="text-[10px] font-medium truncate">
                    {disp?.motivoBloqueo ?? disp?.tipoBloqueo?.replace(/_/g, ' ') ?? 'Bloqueado'}
                  </span>
                </div>
              )}
              ...
              {!bloqueado && !feriado && disp && (
                <>
                  {!esPrivado && tieneReservas && (
                    <div className="px-2 pt-2 space-y-1.5">
                      <div className="h-1 w-full rounded-full bg-gray-100">
                        <div
                          className={cn('h-1 rounded-full transition-all', barColor(pct))}
                  ...
                  {!esPrivado && !tieneReservas && !pasado && (
                    <div className="px-2 pt-2">
                      <span className="text-[10px] text-gray-300 font-medium">Sin actividad</span>
                    </div>
                  )}
              ...
              {!disp && !isLoading && (
                <div className="px-2 pt-2 divide-y divide-gray-100 mt-auto">
                  <TurnoSlot icon={Sun} label="Mañana" titulo={null} ocupado={false} pasado={pasado} />
                  <TurnoSlot icon={Sunset} label="Tarde" titulo={null} ocupado={false} pasado={pasado} />
                </div>
              )}
            </div>
          </div>
