    ...
    >
      ...
      {turno.disponible && !turno.eventoPrivado && permitirAsignar && (
        <Button
          size="sm"
          variant="outline"
          className="h-7 text-xs px-3 rounded-lg gap-1.5 w-full"
          asChild
        >
      ...
      {turno.eventoPrivado && (
        <div className="text-xs text-gray-600 space-y-1">
          <p className="font-semibold text-gray-800">
            {turno.eventoPrivado.tipoEvento}
          </p>
...
export const CalendarioDia = React.memo(
  ({ fecha, idSede }: CalendarioDiaProps) => {
          ...
          {esPasado ? (
            <div className="flex gap-2">
              <Button
                size="sm"
                variant="outline"
                className="border-brand-azul/30 text-brand-azul hover:bg-brand-azul/5 rounded-xl gap-1.5 text-xs"
                asChild
              >
              ...
              <Button
                size="sm"
                variant="outline"
                className="border-brand-rosa/30 text-brand-rosa hover:bg-brand-rosa/5 rounded-xl gap-1.5 text-xs"
                asChild
              >
      ...
      <>
          ...
          <div className="lg:col-span-2 space-y-4">
            {esPasado && (
              <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3">
                ...
                <p className="text-xs font-semibold text-gray-500">
                  Fecha historica. Puedes consultar pero no realizar acciones
                  operativas.
                </p>
              </div>
            )}
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  },
                  {
                    icon: TrendingUp,
                    label: 'Ingresos',
                    value: formatCurrency(resumen.ingresoEstimado, 0),
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between text-xs text-gray-500">
                    ...
                    <span className="font-semibold">
                      {resumen.aforoPublicoActual} / {resumen.aforoMaximo}
                    </span>
                  </div>
            ...
            {!bloqueado && resumen.totalReservas === 0 && (
              <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-3">
                ...
                <TurnoSection
                  turno={resumen.turnoT1}
                  label="Mañana"
                  horario={fmtTurno(config?.turnoT1Inicio, config?.turnoT1Fin)}
                  turnoKey="T1"
                  fecha={fechaStr}
                  esPasado={esPasado}
                ...
                />
                <TurnoSection
                  turno={resumen.turnoT2}
                  label="Tarde"
                  horario={fmtTurno(config?.turnoT2Inicio, config?.turnoT2Fin)}
                  turnoKey="T2"
                  fecha={fechaStr}
                  esPasado={esPasado}
                  esSemanaBloqueada={esSemanaBloqueada}
                ...
                <div className="space-y-2">
                  {resumen.totalReservas > 0 && (
                    <Button
                      size="sm"
                      variant="outline"
                      className="border-brand-azul/30 text-brand-azul hover:bg-brand-azul/5 rounded-xl gap-1.5 text-xs w-full"
                      asChild
                    >
                      <Link href={`/admin/reservas?fecha=${fechaStr}`}>
                        <Ticket className="h-3.5 w-3.5" />
                  ...
                  {resumen.totalEventos > 0 && (
                    <Button
                      size="sm"
                      variant="outline"
                      className="border-brand-rosa/30 text-brand-rosa hover:bg-brand-rosa/5 rounded-xl gap-1.5 text-xs w-full"
                      asChild
                    >
                      <Link href={`/admin/eventos?fecha=${fechaStr}`}>
                        <PartyPopper className="h-3.5 w-3.5" />
                ...
                <div className="space-y-2">
                  {!esSemanaBloqueada &&
                    ...
                    resumen.totalReservas === 0 && (
                      <div className="flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-xs text-amber-700">
                        ...
                        Semana sin programación. Las reservas públicas no están
                        habilitadas.
                      </div>
                    )}
                  {esSemanaBloqueada &&
                    !(disp?.disponiblePublico ?? false) &&
                    !esPrivado &&
                    resumen.totalReservas === 0 && (
                      <div className="flex items-start gap-2 rounded-xl border border-blue-200 bg-blue-50 px-3 py-2.5 text-xs text-blue-700">
                        ...
                        Fecha fuera de la ventana de anticipación. Los clientes
                        no pueden reservar este día.
                      </div>
                    )}
                  {!esPrivado && (disp?.disponiblePublico ?? false) && (
                    <Button
                      size="sm"
                      className="bg-brand-azul hover:bg-brand-azul/90 text-white rounded-xl gap-1.5 text-xs w-full"
                      asChild
                    >
                      <Link href="/admin/ventas/nueva">
                        <Ticket className="h-3.5 w-3.5" />
                  ...
                  {(disp?.disponiblePrivado ?? false) && (
                    <Button
                      size="sm"
                      variant="outline"
                      className="border-brand-rosa text-brand-rosa hover:bg-brand-rosa/5 rounded-xl gap-1.5 text-xs w-full"
                      asChild
                    >
                      <Link href={`/admin/eventos/nuevo?fecha=${fechaStr}`}>
                        <PartyPopper className="h-3.5 w-3.5" />
                  ...
                  <Button
                    ...
                    title={
                      tieneAtencionActiva
                        ? 'No se puede bloquear un dia con atencion activa'
                        : ''
                    }
                  >
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-amber-700">
                    {formatCurrency(resumen.pagosPendientes)} por cobrar
                  </p>
                </div>
              </div>
            )}
        ...
        {!esPasado && (
          <ConfirmDialog
            ...
            confirmLabel={
              tieneActividad ? 'Bloquear de todos modos' : 'Bloquear'
            }
