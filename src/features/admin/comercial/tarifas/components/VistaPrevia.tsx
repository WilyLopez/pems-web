  ...
  const precios = [
    {
      ...
      icon: Clock,
      precio: precioSemana != null ? `S/ ${precioSemana.toFixed(2)}` : 'Sin configurar',
      badge: 'Tarifa regular',
      badgeClass: 'bg-brand-azul/10 text-brand-azul border-brand-azul/20 hover:bg-brand-azul/10',
    ...
    },
    {
      tipo: 'Fin de Semana',
      icon: Zap,
      precio: precioFinDeSemana != null ? `S/ ${precioFinDeSemana.toFixed(2)}` : 'Sin configurar',
      badge: 'Shows incluidos',
      badgeClass: 'bg-brand-rosa/10 text-brand-rosa border-brand-rosa/20 hover:bg-brand-rosa/10',
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm">
        <div>
          <h3 className="text-lg font-black text-gray-900">Vista previa del sitio público</h3>
          <p className="text-sm text-gray-500 mt-1">
            Así es como los clientes verán las tarifas de tu sede en la página web.
          </p>
        </div>
        <div className="grid gap-6 sm:grid-cols-2 mt-6">
          {precios.map(({ tipo, icon: Icon, precio, badge, badgeClass, borderClass, desc, iconColor }) => (
            <div
              key={tipo}
            ...
            >
              <div>
                ...
                <Badge variant="outline" className={`mt-2 text-xs font-semibold ${badgeClass}`}>
                  {badge}
                </Badge>
              </div>
