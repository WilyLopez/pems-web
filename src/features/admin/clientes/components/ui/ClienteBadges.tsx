...
import { OrigenCliente, SegmentoCliente } from '../../types'
...
export function OrigenBadge({ origen }: { origen?: OrigenCliente | null }) {
  if (origen === 'MOSTRADOR') {
    return (
      <Badge
        variant="outline"
        className="gap-1 text-xs text-orange-600 border-orange-200 bg-orange-50"
      >
  ...
  if (origen === 'ADMIN') {
    return (
      <Badge
        variant="outline"
        className="gap-1 text-xs text-purple-600 border-purple-200 bg-purple-50"
      >
  ...
  return (
    <Badge
      variant="outline"
      className="gap-1 text-xs text-brand-azul border-brand-azul/20 bg-brand-azul/5"
    >
...
const SEGMENTO_CONFIG: Record<SegmentoCliente, { label: string; cls: string }> =
  {
    ...
    VIP: {
      label: 'VIP',
      cls: 'text-yellow-700 border-yellow-200 bg-yellow-50',
    },
    CORPORATIVO: {
      label: 'Corporativo',
    ...
    INACTIVO: {
      label: 'Inactivo',
      cls: 'text-gray-400 border-gray-100 bg-gray-50',
    },
  }
