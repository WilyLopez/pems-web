...
import { z } from 'zod'
import { usePresupuestosEvento, usePresupuestoMutations } from '../hooks/useFinanceData'
import { presupuestoNuevoSchema, presupuestoEjecutarSchema } from '../schemas/finance.schemas'
...
export function PresupuestoEventoSection({ idEvento }: Props) {
  ...
  const formNuevo    = useForm<NuevoForm>({ resolver: zodResolver(presupuestoNuevoSchema) })
  const formEjecutar = useForm<EjecutarForm>({ resolver: zodResolver(presupuestoEjecutarSchema) })
