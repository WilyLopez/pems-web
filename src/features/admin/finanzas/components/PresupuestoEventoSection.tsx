...
import { z } from 'zod'
import {
  usePresupuestosEvento,
  usePresupuestoMutations,
} from '../hooks/useFinanceData'
import {
  presupuestoNuevoSchema,
  presupuestoEjecutarSchema,
...
type NuevoForm = z.infer<typeof presupuestoNuevoSchema>
type EjecutarForm = z.infer<typeof presupuestoEjecutarSchema>
...
export function PresupuestoEventoSection({ idEvento }: Props) {
  ...
  const formNuevo = useForm<NuevoForm>({
    resolver: zodResolver(presupuestoNuevoSchema),
  })
  const formEjecutar = useForm<EjecutarForm>({
    resolver: zodResolver(presupuestoEjecutarSchema),
  })
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-4 text-xs">
          ...
          {totalEstimado > 0 && (
            <span
              className={cn(
                'font-semibold',
                totalReal > totalEstimado ? 'text-red-500' : 'text-brand-azul'
              )}
            >
            ...
            >
              ...
              <div className="text-right shrink-0 space-y-0.5">
                <p className="text-xs text-gray-500">
                  Est. {formatCurrency(item.montoEstimado)}
                </p>
                {item.montoReal != null && (
                  <p className="text-xs font-semibold text-emerald-600">
                    Real {formatCurrency(item.montoReal)}
                  </p>
                )}
              </div>
              <span
              ...
              <div className="flex items-center gap-2 shrink-0">
                {item.estado !== 'EJECUTADO' && (
                  <button
                    type="button"
                    onClick={() => {
                      setEjecutandoId(item.id)
                      formEjecutar.setValue('montoReal', item.montoEstimado)
                    }}
      ...
      <Dialog open={modalNuevo} onOpenChange={setModalNuevo}>
        <DialogContent className="sm:max-w-md">
          ...
          <form
            onSubmit={formNuevo.handleSubmit(onGuardar)}
            className="space-y-4 pt-2"
          >
            <div className="space-y-1">
              <Label>Concepto</Label>
              <Input
                {...formNuevo.register('concepto')}
                placeholder="Ej. Decoración"
              />
              {formNuevo.formState.errors.concepto && (
                <p className="text-xs text-red-500">
                  {formNuevo.formState.errors.concepto.message}
                </p>
              )}
            </div>
            <div className="space-y-1">
              ...
              {formNuevo.formState.errors.categoria && (
                <p className="text-xs text-red-500">
                  {formNuevo.formState.errors.categoria.message}
                </p>
              )}
            </div>
            <div className="space-y-1">
              ...
              <Input
                ...
                step="0.01"
                min="0"
                {...formNuevo.register('montoEstimado')}
              />
              {formNuevo.formState.errors.montoEstimado && (
                <p className="text-xs text-red-500">
                  {formNuevo.formState.errors.montoEstimado.message}
                </p>
              )}
            </div>
      ...
      <Dialog
        open={ejecutandoId !== null}
        onOpenChange={(v) => {
          if (!v) setEjecutandoId(null)
        }}
      >
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>Registrar monto real</DialogTitle>
          </DialogHeader>
          <form
            onSubmit={formEjecutar.handleSubmit(onEjecutar)}
            className="space-y-4 pt-2"
          >
            <div className="space-y-1">
              ...
              <Input
                ...
                step="0.01"
                min="0"
                {...formEjecutar.register('montoReal')}
              />
              {formEjecutar.formState.errors.montoReal && (
                <p className="text-xs text-red-500">
                  {formEjecutar.formState.errors.montoReal.message}
                </p>
              )}
            </div>
