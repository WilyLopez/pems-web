...
import { cambiarSchema, CambiarFormValues } from '../../schemas/auth.schema'
...
export function CambiarContrasenaView() {
  ...
  const cambiarMutation = useCambiarContrasena({
    onSuccess: () => {
      setExito(true)
    },
    onError: (err: any) => {
  ...
  const {
    ...
    setError,
    formState: { errors },
  } = useForm<CambiarFormValues>({ resolver: zodResolver(cambiarSchema) })

  const nuevaContrasena = watch('nuevaContrasena') ?? ''

  const onSubmit = (values: CambiarFormValues) => {
    cambiarMutation.mutate(values)
  }
