...
interface Props {
  ...
  total: number
  onSalir: () => void
  /** Seconds remaining in the session timer */
  secondsLeft: number
  /** 0–1 progress, 1 = full time */
...
export function WizardHeader({ paso, total, onSalir, secondsLeft, timerProgress, timerPhase, timerDisplay }: Props) {
  const styles = phaseStyles[timerPhase]
  const showTimerWarning = timerPhase === 'warning' || timerPhase === 'critical'
  ...
  return (
    <div className="sticky top-0 z-30 bg-white border-b border-gray-100 shadow-sm">
      ...
      <div className="h-1 w-full bg-gray-100 overflow-hidden">
        <div
          className={cn('h-full transition-all duration-1000 ease-linear', styles.bar, timerPhase === 'critical' && 'animate-pulse')}
      ...
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Mobile: only show timer if warning/critical */}
          {showTimerWarning && (
