                ...
                <div className="text-left">
                  <p
                    className={cn(
                      ...
                      isActive ? 'text-brand-azul' : isCompleted ? 'text-green-600' : 'text-gray-400'
                    )}
                  >
                    {p.label}
                  </p>
                  <p className="text-[10px] text-gray-400 mt-1 font-medium">{p.desc}</p>
                </div>
              </div>
...
export function ReservarView() {
  ...
  useEffect(() => {
    ...
    if (draft) {
      try {
        ...
        if (parsed.nombreAcompanante) setValue('nombreAcompanante', parsed.nombreAcompanante)
        if (parsed.dniAcompanante) setValue('dniAcompanante', parsed.dniAcompanante)
      } catch (e) {
  ...
  // Save draft
  useEffect(() => {
    if (watchedValues.nombreNino || watchedValues.edadNino || watchedValues.nombreAcompanante || watchedValues.dniAcompanante) {
    ...
    return (
      <div className="container max-w-xl mx-auto px-4 pt-24 pb-12">
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 text-center space-y-6">
          ...
          <div>
            <h2 className="text-2xl font-black text-gray-900 leading-tight">Reserva confirmada</h2>
          ...
          <div className="bg-gray-50 rounded-2xl p-5 text-left space-y-4 border border-gray-100">
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-400 font-mono uppercase tracking-wider">Ticket</span>
            ...
            <div className="border-t border-gray-200/80 pt-3 space-y-2.5">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-gray-800">{reservaCreada.nombreNino}</span>
              </div>
              <div className="flex justify-between text-sm">
          ...
          <div className="flex flex-col gap-3 pt-2">
            <button
              onClick={async () => {
                try {
                  await downloadFile(`/reservas/${reservaCreada.id}/ticket`, `ticket-${reservaCreada.numeroTicket}.pdf`)
            ...
            <div className="space-y-5">
              <div className="mb-2">
                ...
                <p className="text-xs text-gray-405 mt-1 leading-relaxed font-semibold">
                  Puedes reservar hasta con {diasMax} días de anticipación. Atención de {horaApertura} a {horaCierre}.
                </p>
              </div>
                ...
                {loadingDisp ? (
                  ...
                  <div className="grid grid-cols-7 gap-1 sm:gap-2">
                    {dias.map((dia) => {
                      ...
                      const minReservaDia = startOfDay(addDays(hoy, config?.diasMinReservaPublica ?? 0))
                      const maxReservaDia = startOfDay(addDays(hoy, config?.diasMaxReservaPublica ?? 14))
                      const fueraDeRango = isBefore(dia, minReservaDia) || isAfter(dia, maxReservaDia)
                      
                      const disabled = pasado || fueraDeRango || !disp || !disp.disponiblePublico || cerroHoy
                            ...
                            <span className="text-[8px] xs:text-[9px] font-black text-brand-azul mt-auto leading-none">
                              S/{Number(
                                precioMap
                                  ...
                                  : (getTarifaKey(fechaStr, disp.esFeriado) === 'SEMANA' ? 25 : 35)
                              ).toFixed(0)}
                            </span>
                          )}
              ...
              {fechaSeleccionada && dispSeleccionada && (
                <div className="p-5 bg-brand-azul/5 border border-brand-azul/20 rounded-3xl space-y-4 shadow-sm animate-fade-in">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <PrecioLabel tipoDia={getTarifaKey(fechaSeleccionada, dispSeleccionada.esFeriado)} precioMap={precioMap} />
                  </div>
                  <button
                    onClick={() => setPaso(2)}
              ...
              <div className="bg-white rounded-3xl border border-gray-100 p-5 shadow-sm">
                ...
                <ol className="space-y-2.5 text-xs text-gray-500 list-none">
                  {['Elige la fecha disponible en el calendario.', 'Completa los datos del niño y del acompañante adulto.', 'Selecciona tu método de pago preferido (Yape o Pago en Local).', 'Recibe tu ticket de ingreso con código QR en formato PDF.'].map((s, i) => (
          ...
          {paso === 2 && (
            <div className="space-y-5">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  <h1 className="text-xl font-black text-gray-900 tracking-tight">Datos del visitante</h1>
                  {fechaSeleccionada && (
              ...
              <form onSubmit={handleSubmit(() => setPaso(3))} className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="md:col-span-2 space-y-1.5">
                      <label className="text-xs font-bold text-gray-500 uppercase tracking-wide">
                        Nombre completo del niño <span className="text-red-500 font-bold">*</span>
                      </label>
                      <Input
                        placeholder="Valentina Garcia"
                        className={cn('h-11 rounded-xl focus:border-brand-azul', errors.nombreNino && 'border-red-400')}
                        {...register('nombreNino')}
                      />
                      {errors.nombreNino && (
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        max={17}
                        placeholder="5"
                        className={cn('h-11 rounded-xl focus:border-brand-azul', errors.edadNino && 'border-red-400')}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="md:col-span-2 space-y-1.5">
                      ...
                      <Input
                        ...
                        className={cn('h-11 rounded-xl focus:border-brand-azul', errors.nombreAcompanante && 'border-red-400')}
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        maxLength={8}
                        className={cn('h-11 rounded-xl font-mono focus:border-brand-azul', errors.dniAcompanante && 'border-red-400')}
                        {...register('dniAcompanante')}
                      />
                      <p className="text-[10px] text-gray-400">8 dígitos numéricos</p>
          ...
          {paso === 3 && (
            <div className="space-y-5">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-xl font-black text-gray-900 tracking-tight">Método de pago</h1>
              </div>
              ...
              <div className="lg:hidden bg-gray-50 rounded-2xl border border-gray-100 p-4 space-y-2">
                ...
                <p className="text-xs text-gray-600">Niño: {getValues('nombreNino')}</p>
                <div className="border-t border-gray-200/80 pt-2 mt-2 flex justify-between items-center">
                  <span className="text-xs text-gray-500 font-semibold">Total a pagar</span>
                  <PrecioLabel tipoDia={getTarifaKey(fechaSeleccionada!, dispSeleccionada!.esFeriado)} precioMap={precioMap} />
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <div className="p-4 flex-1 flex flex-col justify-between">
                      <div>
                        <p className="font-black text-gray-900 text-base">Pagar con Yape</p>
                        <p className="text-xs text-gray-500 mt-1 leading-relaxed">
                          Transfiere de forma rápida desde tu celular subiendo la captura del comprobante.
                        </p>
                      </div>
                  ...
                  >
                    ...
                    <div className="p-4 flex-1 flex flex-col justify-between">
                      <div>
                        <p className="font-black text-gray-900 text-base">Pagar en Local (Caja)</p>
                        <p className="text-xs text-gray-500 mt-1 leading-relaxed">
                          Asegura tu cupo en línea y realiza el pago con efectivo o tarjeta al ingresar.
                        </p>
                      </div>
                ...
                {metodoPago === 'YAPE' && (
                  <div className="p-5 bg-[#6E2FEC]/5 border border-[#6E2FEC]/20 rounded-3xl space-y-4 shadow-sm animate-fade-in">
                    <p className="text-sm font-black text-purple-950 uppercase tracking-wider">Instrucciones de Pago:</p>
                    <ol className="text-xs text-gray-700 space-y-2 list-decimal list-inside leading-relaxed font-medium">
                      ...
                      <li>Busca el número oficial del local (o escanea el QR en recepción).</li>
                      <li>
                        Envía exactamente{' '}
                      ...
                      <li>Escribe el nombre del niño en el concepto del yapeo.</li>
                      <li>Sube la captura de pantalla del comprobante de pago aquí abajo.</li>
                    </ol>
                ...
                {metodoPago === 'CAJA' && (
                  <div className="p-5 bg-amber-50 border border-amber-200 rounded-3xl flex items-start gap-3 shadow-sm animate-fade-in">
                    ...
                    <p className="text-xs text-amber-800 leading-relaxed font-medium">
                      <strong className="font-bold">Importante:</strong> El cupo quedará reservado temporalmente, pero el ingreso final al local
                      solo se confirmará al efectuar el pago correspondiente en caja. Te aconsejamos llegar 10 minutos antes de tu hora planeada.
                    </p>
                  </div>
                )}
            ...
            {fechaSeleccionada ? (
              <div className="space-y-4">
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Fecha Seleccionada</p>
                  <p className="font-bold text-gray-800 capitalize text-sm">
                    {format(parseISO(fechaSeleccionada), "EEEE d 'de' MMMM yyyy", { locale: es })}
                  </p>
                  <p className="text-[11px] text-gray-400 leading-relaxed">
                    Sede Principal &middot; Horario de atención: {horaApertura} - {horaCierre}
                  </p>
                </div>

                {dispSeleccionada && (
                  <div className="flex justify-between items-center bg-gray-50 rounded-2xl p-3 border border-gray-100">
                    <span className="text-xs text-gray-650 font-bold">Valor Entrada</span>
                    <PrecioLabel tipoDia={getTarifaKey(fechaSeleccionada, dispSeleccionada.esFeriado)} precioMap={precioMap} />
                  </div>
                )}

                {/* Form fields review */}
                {(watchedValues.nombreNino || watchedValues.nombreAcompanante) && (
                  <div className="space-y-2.5 pt-3.5 border-t border-gray-100">
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Datos Registrados</p>
                    {watchedValues.nombreNino && (
                      <div className="text-xs text-gray-750 font-semibold">
                        <span className="text-gray-400 font-medium">Niño:</span>{' '}
                        <strong className="text-gray-800">{watchedValues.nombreNino}</strong>
                        {watchedValues.edadNino !== undefined && !isNaN(Number(watchedValues.edadNino)) && (
                          <span className="text-gray-400"> ({watchedValues.edadNino} años)</span>
                        )}
                      </div>
                    )}
                    {watchedValues.nombreAcompanante && (
                      <div className="text-xs text-gray-755 font-semibold">
                        <span className="text-gray-400 font-medium">Acompañante:</span>{' '}
                        <strong className="text-gray-800">{watchedValues.nombreAcompanante}</strong>
                        {watchedValues.dniAcompanante && (
                          <span className="text-gray-400 font-mono text-[11px]"> (DNI: {watchedValues.dniAcompanante})</span>
                        )}
                      </div>
                    )}
                  </div>
                    ...
                    <div className="flex items-center gap-2 bg-gray-50 rounded-xl p-2.5 border border-gray-100">
                      {metodoPago === 'YAPE' ? (
                        ...
                        <>
                          ...
                          <span className="text-xs font-bold text-gray-800">Pago en Local</span>
                        </>
                      )}
                    </div>
                  </div>
                ...
                <div className="pt-4 border-t border-gray-100 flex justify-between items-end">
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400 font-medium">Impuestos incluidos</p>
                  </div>
              ...
              <div className="text-center py-8 space-y-2.5">
                ...
                <p className="text-xs text-gray-400 font-bold leading-relaxed">
                  Elige una fecha para ver el resumen en tiempo real de tu reserva.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="max-w-[70%]">
              ...
              <p className="text-xs font-black text-gray-800 truncate mt-1 leading-none">
                {watchedValues.nombreNino || 'Visitante'} &middot; {format(parseISO(fechaSeleccionada), "d 'de' MMMM", { locale: es })}
              </p>
            </div>
