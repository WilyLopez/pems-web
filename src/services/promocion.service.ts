...
export const promocionService = {
  ...
  crear: async (payload: CrearPromocionPayload): Promise<Promocion> => {
    const { data } = await api.post<ApiResponse<Promocion>>(
      '/promociones',
      payload
    )
    return data.data
  ...
  actualizar: async (
    ...
    const { data } = await api.put<ApiResponse<Promocion>>(
      `/promociones/${id}`,
      payload
    )
    return data.data
  ...
  estadisticas: async (): Promise<EstadisticasPromocion> => {
    const { data } = await api.get<ApiResponse<EstadisticasPromocion>>(
      '/promociones/estadisticas'
    )
    return data.data
  },
}
